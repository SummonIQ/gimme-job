...
export const UNSUPPORTED_RUNTIME_PROVIDER: RuntimeProvider = {
  id: 'unsupported',
  label: 'Unsupported',
...
export function getRuntimeProviderForUrl(url: string): RuntimeProvider {
  const candidates = getProviderMatchCandidates(url);
  if (!candidates) return UNSUPPORTED_RUNTIME_PROVIDER;
  return (
    RUNTIME_PROVIDERS.find(provider =>
