...
const TALEO_CONTINUE_SELECTORS: readonly string[] = [
...
const TALEO_SUBMIT_SELECTORS: readonly string[] = [
...
const TALEO_AGREE_SELECTORS: readonly string[] = [
...
export async function runTaleoSubmitLead(
...
export function isTaleoApplicationUrl(url: string): boolean {
  return /\.taleo\.net|tbe\.taleo\.net|taleocloud\.com/i.test(url);
}

function createTaleoRuntime(
  request: DesktopSubmitLeadRequest,
...
async function runTaleoRuntime(
  request: DesktopSubmitLeadRequest,
  ...
  if (overlaysDismissed > 0) {
    console.log(
      `[taleo-submit] dismissed ${overlaysDismissed} consent overlay(s)`,
    );
  }
  ...
  for (let iteration = 0; iteration < MAX_PAGE_ITERATIONS; iteration += 1) {
    ...
    if (!identityFilled) {
      const filledFirst = await tryFill(
        input,
        TALEO_IDENTITY_SELECTORS.firstName,
        identity.firstName,
        'Taleo first name',
        attemptedSelectors,
        root,
      );
      if (filledFirst) identityFilled = true;
      await tryFill(
        input,
        TALEO_IDENTITY_SELECTORS.lastName,
        identity.lastName,
        'Taleo last name',
        attemptedSelectors,
        root,
      ...
      await tryFill(
        input,
        TALEO_IDENTITY_SELECTORS.email,
        identity.email,
        'Taleo email',
        attemptedSelectors,
        root,
      ...
      await tryFill(
        input,
        TALEO_IDENTITY_SELECTORS.phone,
        identity.phone,
        'Taleo phone',
        attemptedSelectors,
        root,
    ...
    if (request.mode !== 'submit' && reviewLikely) {
      return {
        message:
          'Training run reached Taleo review page (submit not clicked).',
        status: 'completed',
      };
    }
