...
const SUBMIT_BUTTON_SELECTOR =
  'button[type="submit"], input[type="submit"], button[data-test*="submit" i], button[data-qa*="submit" i], button[aria-label*="submit" i]';

const SUBMIT_BUTTON_TEXT_PATTERN =
  /^(submit|submit application|apply|apply now|send application|continue|next)$/i;

const ANY_INPUT_SELECTOR =
...
export interface GenericSubmitProviderOverrides {
  readonly providerLabel: string;
  // Direct selectors that should be tried first for each identity field
  // before falling through to the regex-based label/name matcher. Ashby /
  // Lever / Workable each expose stable system-field names — these let
  // us hit the right input on the first shot.
  readonly identitySelectors?: {
  ...
  readonly resumeFileSelectors?: readonly string[];
  readonly submitButtonSelectors?: readonly string[];
}

export async function runGenericSubmitLead(
  registry: DesktopToolRegistry,
  request: DesktopSubmitLeadRequest,
  options: {
    readonly signal?: AbortSignal;
    readonly providerOverrides?: GenericSubmitProviderOverrides;
  } = {},
): Promise<DesktopSubmitLeadResult> {
  const session = createDesktopAgentSession(registry, {
    mode: request.mode,
    runtime: createClaudeAgentSdkRuntime(
      createGenericRuntime(request, options.providerOverrides),
    ),
  });
...
function buildObjective(
  request: DesktopSubmitLeadRequest,
  overrides?: GenericSubmitProviderOverrides,
): string {
...
async function runGenericRuntime(
  request: DesktopSubmitLeadRequest,
  ...
  const readyResult = await call(
    input,
    'wait_for',
    { selector: ANY_INPUT_SELECTOR, timeoutMs: 15_000 },
    'wait for application form to render',
  );
  ...
  if (overlaysDismissed > 0) {
    console.log(
      `[generic-submit] dismissed ${overlaysDismissed} consent overlay(s)`,
    );
  }
  ...
  const filledFullName = await fillBySelectorOverrides(
    input,
    fields,
    attemptedSelectors,
    fullNameOverrides,
  ...
  if (!filledFullName) {
    await fillBySelectorOverrides(
      input,
      fields,
      attemptedSelectors,
      overrides?.identitySelectors?.firstName ?? [],
      identity.firstName,
      'fill first name',
    );
    await fillIfMatch(
    ...
    await fillIfMatch(
      ...
      fields,
      attemptedSelectors,
      IDENTITY_PATTERNS.preferredName,
      identity.firstName,
      'fill preferred name',
    );
    await fillBySelectorOverrides(
      ...
      fields,
      attemptedSelectors,
      overrides?.identitySelectors?.lastName ?? [],
      identity.lastName,
      'fill last name',
    );
    await fillIfMatch(
  ...
  await fillBySelectorOverrides(
    ...
    fields,
    attemptedSelectors,
    overrides?.identitySelectors?.email ?? [],
    identity.email,
    'fill email',
  );
  await fillIfMatch(
  ...
  await fillBySelectorOverrides(
    ...
    fields,
    attemptedSelectors,
    overrides?.identitySelectors?.phone ?? [],
    identity.phone,
    'fill phone',
  );
  await fillIfMatch(
  ...
  if (contactUrls.linkedinUrl) {
    await fillIfMatch(
      ...
      fields,
      attemptedSelectors,
      URL_FIELD_PATTERNS.linkedin,
      contactUrls.linkedinUrl,
      'fill linkedin url',
    );
  }
  if (contactUrls.githubUrl) {
    await fillIfMatch(
      ...
      fields,
      attemptedSelectors,
      URL_FIELD_PATTERNS.github,
      contactUrls.githubUrl,
      'fill github url',
    );
  }
  if (contactUrls.websiteUrl) {
    await fillIfMatch(
      ...
      fields,
      attemptedSelectors,
      URL_FIELD_PATTERNS.website,
      contactUrls.websiteUrl,
  ...
  const resumeSelector =
    findFirstMatchingSelector(html, overrides?.resumeFileSelectors) ??
    findResumeFileInputSelector(html);
  if (resumeSelector) {
    const uploadResult = await call(
      input,
      'upload',
      { filePath: resumeFilePath, selector: resumeSelector },
      'attach resume',
    );
    if (uploadResult.ok) {
  ...
  // interaction, etc.).
  const finalSnapshot = await call(
    input,
    'dom_snapshot',
    {},
    'snapshot before submit click',
  );
  ...
  const submitSelector =
    findFirstMatchingSelector(
      submitSearchHtml,
      overrides?.submitButtonSelectors,
    ) ?? findSubmitButtonSelector(submitSearchHtml);
  if (!submitSelector) {
    return {
      message:
        'Could not locate a submit button on this generic form (no [type="submit"], no button text matching submit/apply/send).',
      status: 'validation_failed',
    };
  }
  const unavailableReason = getSubmitButtonUnavailableReason(
    submitSearchHtml,
    submitSelector,
  );
  ...
  const guardResult = await call(
    ...
    'submit_guard',
    { enabled: false },
    'owner-approved submit mode (generic)',
  );
  ...
  const submitResult = await call(
    ...
    'submit application (generic)',
  );
  if (!submitResult.ok) return failed(submitResult);
  ...
  if (!confirmation.confirmed) {
    ...
    return {
      message: `Submit click fired but no confirmation page appeared within ${confirmation.timeoutMs}ms.${
...
async function extractCurrentValidationFailures(
  input: DesktopAgentRuntimeInput,
): Promise<ReturnType<typeof extractValidationFailures>> {
  const snapshot = await call(
    input,
    'dom_snapshot',
    {},
...
async function fillBySelectorOverrides(
  input: DesktopAgentRuntimeInput,
  ...
  for (const candidate of selectors) {
    if (attempted.has(candidate)) return true;
  ...
  if (attempted.has(first)) return true;
  attempted.add(first);
...
function findFirstMatchingSelector(
  html: string,
...
async function fillIfMatch(
  ...
  const match = fields.find(field => {
    if (attempted.has(field.selector)) return false;
    const haystack =
      `${field.labelText} ${field.name} ${field.id} ${field.placeholder}`.toLowerCase();
...
function findResumeFileInputSelector(html: string): string | null {
  const root = parse(html);
...
function selectorForFileInput(id: string, name: string): string {
  if (id) return `input#${cssEscape(id)}[type="file"]`;
  if (name) return `input[name="${cssEscapeAttr(name)}"][type="file"]`;
  return 'input[type="file"]';
}

function findSubmitButtonSelector(html: string): string | null {
  const root = parse(html);
  ...
  if (strict) {
    ...
    if (id) return `${strict.tagName.toLowerCase()}#${cssEscape(id)}`;
  ...
  for (const button of buttons) {
    ...
    if (SUBMIT_BUTTON_TEXT_PATTERN.test(text)) {
      ...
      if (id) return `button#${cssEscape(id)}`;
...
function getSubmitButtonUnavailableReason(
  html: string,
  selector: string,
): 'submit_button_disabled' | null {
  const root = parse(html);
  let element: ReturnType<typeof root.querySelector>;
  try {
  ...
  if (element.hasAttribute('disabled')) return 'submit_button_disabled';
  if (element.getAttribute('aria-disabled')?.toLowerCase() === 'true') {
    return 'submit_button_disabled';
  }
  if (element.hasAttribute('hidden')) return 'submit_button_disabled';
  const style = element.getAttribute('style')?.toLowerCase() ?? '';
  if (/display\s*:\s*none/.test(style)) return 'submit_button_disabled';
  if (/visibility\s*:\s*hidden/.test(style)) return 'submit_button_disabled';
  return null;
}

function collectGenericFieldCandidates(
  html: string,
): readonly FieldCandidate[] {
  const root = parse(html);
  ...
  for (const candidate of candidates) {
    ...
    let selector: string;
    if (id) selector = `${candidate.tagName.toLowerCase()}#${cssEscape(id)}`;
    else if (name)
    ...
    if (id) {
      ...
      if (label?.text) labelText = label.text.trim();
    }
