 ...
 * Greenhouse posting.
 */
export function isGreenhouseApplicationUrl(url: string): boolean {
  if (typeof url !== 'string' || url.length === 0) return false;
  if (/greenhouse\.io|job-boards\.greenhouse/i.test(url)) return true;
  // gh_jid as a query / hash param. Anchored on `?` `&` or `#` so bare
  // `gh_jid` substring inside a path can't false-match.
  if (/[?&#]gh_jid=/i.test(url)) return true;
  return false;
}
