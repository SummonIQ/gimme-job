...
describe('getRuntimeProviderForUrl', () => {
  ...
  it('blocks autopilot for unsupported and manual-review providers', () => {
    expect(
      shouldBlockAutopilotForProvider(
        getRuntimeProviderForUrl('https://example.com/jobs/123'),
      ),
    ).toBe(true);
    expect(
      shouldBlockAutopilotForProvider(
  ...
  it('allows training autofill to fall through for unsupported providers', () => {
    expect(
      shouldBlockAutopilotForProvider(
        getRuntimeProviderForUrl('https://example.com/jobs/123'),
        'training',
