...
export const DESKTOP_RUNTIME_PROVIDERS: readonly DesktopRuntimeProvider[] = [
  {
    id: 'greenhouse',
    label: 'Greenhouse',
...
export const UNSUPPORTED_RUNTIME_PROVIDER: DesktopRuntimeProvider = {
  id: 'unsupported',
  label: 'Unsupported',
...
export function getRuntimeProviderForUrl(
  url: string,
): DesktopRuntimeProvider {
  const candidates = getProviderMatchCandidates(url);
  if (!candidates) return UNSUPPORTED_RUNTIME_PROVIDER;
  return (
    DESKTOP_RUNTIME_PROVIDERS.find(provider =>
...
function getProviderMatchCandidates(url: string): readonly string[] | null {
  try {
...
export function shouldBlockAutopilotForProvider(
  provider: DesktopRuntimeProvider,
  mode: 'submit' | 'training' = 'submit',
): boolean {
  if (mode === 'training') return false;
  return (
    provider.readiness === 'unsupported' ||
