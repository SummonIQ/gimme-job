...
import {
  DesktopAgentCancelledError,
  type DesktopAgentRunInput,
  type DesktopAgentRunResult,
  type DesktopAgentSessionOptions,
  type DesktopAgentSessionStatus,
  type DesktopAgentToolCall,
  type DesktopAgentToolDefinition,
  type DesktopAgentToolEvent,
} from './types.js';

export interface DesktopAgentSession {
  run: (input: DesktopAgentRunInput) => Promise<DesktopAgentRunResult>;
}

const TOOL_DESCRIPTIONS: Record<DesktopToolName, string> = {
...
export function createDesktopAgentSession(
  ...
  { mode = 'training', runtime }: DesktopAgentSessionOptions,
): DesktopAgentSession {
  return {
    async run({ objective, signal }) {
      const events: DesktopAgentToolEvent[] = [];
...
function createToolDefinitions(
  toolNames: readonly DesktopToolName[],
): DesktopAgentToolDefinition[] {
...
function createRunResult(input: DesktopAgentRunResult): DesktopAgentRunResult {
  return input;
}

function resolveStatus(
  events: readonly DesktopAgentToolEvent[],
  runtimeStatus: DesktopAgentSessionStatus | undefined,
...
function isSubmitGuardBlock(result: DesktopToolCallResult): boolean {
  return (
    !result.ok &&
