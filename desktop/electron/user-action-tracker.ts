...
export interface UserActionFieldEntry {
  readonly id: string;
...
export interface UserActionReport {
  readonly aiBaseline: readonly UserActionFieldEntry[];
  readonly capturedAt: string;
  readonly emptyFields: readonly UserActionFieldEntry[];
  readonly trigger: 'submit' | 'manual';
  readonly unchangedFields: readonly UserActionFieldEntry[];
  readonly url: string;
  ...
  readonly userFilledFields: readonly UserActionFieldEntry[];
}
...
export async function injectUserActionTracker(
...
export function parseUserActionReport(message: string): UserActionReport | null {
  if (!message.startsWith(REPORT_PREFIX)) return null;
  const json = message.slice(REPORT_PREFIX.length);
  try {
    return JSON.parse(json) as UserActionReport;
  } catch (error) {
