...
if (
  ...
  process.env.PLAYWRIGHT_BROWSERS_PATH = pathMod.join(
...
import {
  collectAssistPageContext,
  highlightAssistPageField,
...
import {
  createDesktopSubmitClient,
  type DesktopGreenhouseLeadSearchQuery,
  type DesktopRandomGreenhouseLead,
} from './submit/client.js';
import {
  detectClosedPosting,
  type ClosedPostingVerdict,
} from './submit/closed-posting-detector.js';
import { detectConfirmation as detectProviderConfirmation } from './submit/confirmation-detector.js';
import {
  inferGenderFromFirstName,
  runGreenhouseSubmitLead,
  setAutofillPaused as setGreenhouseAutofillPaused,
  type DesktopSubmitLeadRequest,
  type DesktopSubmitLeadResult,
} from './submit/greenhouse-submit.js';
...
import {
  DESKTOP_RUNTIME_PROVIDERS,
  getRuntimeProviderForUrl,
  shouldBlockAutopilotForProvider,
  type DesktopRuntimeProvider,
} from './submit/provider-registry.js';
import {
  createRunId,
  createRunJsonlToolRegistry,
  openRunLog,
  type RunLog,
} from './submit/run-jsonl-log.js';
import {
  addFieldRule,
  configureFieldRuleSync,
  findMatchingFieldRule,
  getAllFieldRules,
  hydrateRulesFromServer,
  promoteFieldRules,
  removeFieldRule,
} from './field-rules-store.js';
import {
  DESKTOP_SUBMIT_IPC_EVENTS,
  registerDesktopSubmitIpc,
  type DesktopSmokeTestProgressEvent,
  type DesktopSmokeTestRequest,
  type DesktopSmokeTestResult,
  type DesktopSmokeTestRun,
  type DesktopSubmitRunner,
} from './submit/ipc.js';
...
import {
  injectUserActionTracker,
  parseUserActionReport,
} from './user-action-tracker.js';
import {
...
const shellState = createDesktopShellState({
  ...
  assistUrl:
    process.env.GIMME_JOB_ASSIST_URL ??
    persistedAssistUrl ??
...
const toolRegistry = createDesktopToolRegistry(
  createElectronCdpToolDriver({
    getWebContents: () => {
      if (!assistView) {
...
let assistView: BrowserView | null = null;
let appBrowserView: BrowserView | null = null;
...
let lastSubmitResult: Awaited<
  ReturnType<typeof runGreenhouseSubmitLead>
> | null = null;
let activeRunController: AbortController | null = null;
let smokeTestController: AbortController | null = null;
const activeSubmitRunKeys = new Set<string>();

// Verification-code cache. Polled every 30s in the background once the
// desktop is paired so that when the agent hits a Greenhouse "enter the
// 8-digit code" challenge, the most recent code from the user's inbox is
// already on hand instead of round-tripping to /api/desktop/verification-code
// from inside the form-fill loop. The cache stores the response per
// digit-count (Greenhouse forms vary: 6, 7, 8) plus the unspecified
// "any digits" lookup so we don't conflate them.
type CachedVerificationCode = {
...
const verificationCodeCache = new Map<
  string,
  { readonly cachedAt: number; readonly result: CachedVerificationCode | null }
>();
let verificationCodePollTimer: NodeJS.Timeout | null = null;

function verificationCacheKey(digits?: number): string {
  return typeof digits === 'number' && Number.isFinite(digits)
    ? `d:${digits}`
    : 'any';
}

async function lookupVerificationCodeCached(
  ...
  try {
    ...
    if (entry) return entry.result; // Stale-while-error fallback
    throw error;
  }
}

async function pollVerificationCodesOnce(): Promise<void> {
  ...
  for (const digits of [undefined, 8] as const) {
    try {
      const result = await submitClient.lookupRecentVerificationCode(
        digits,
        {},
      );
      const key = verificationCacheKey(digits);
      ...
      if (result) {
        console.log(
          `[verification-poll] cached ${result.digits}-digit code from ${result.fromEmail || 'inbox'}`,
        );
      }
    } catch {
...
function startVerificationCodePoller(): void {
  if (verificationCodePollTimer) return;
...
const agentChatClient = createDesktopAgentChatClient({
  ...
  fillAssistField: async ({ selector, value, kind }) => {
    ...
    if (kind === 'typeahead') {
      ...
      if (!filled.ok) return { ok: false, error: filled.error?.message };
...
function buildSubmitRunKey(input: { readonly applicationUrl: string }): string {
  try {
...
registerDesktopIpc(ipcMain, shellState, {
  assistGoBack: () => {
    ...
    if (webContents && webContents.canGoBack()) webContents.goBack();
    return {
  ...
  assistGoForward: () => {
    const webContents = assistView?.webContents;
    if (webContents && webContents.canGoForward()) webContents.goForward();
    return {
      canGoBack: webContents?.canGoBack() ?? false,
      canGoForward: webContents?.canGoForward() ?? false,
    };
  ...
  readSmokeReport: (filePath: string) =>
    readSmokeReport({
      filePath,
  ...
  listRuntimeProviders: () =>
    DESKTOP_RUNTIME_PROVIDERS.map(provider => ({
      id: provider.id,
      label: provider.label,
      readiness: provider.readiness,
      runner: provider.runner,
    })),
  detectRuntimeProvider: (url: string) => {
  ...
  setAssistField: async ({ kind, selector, value, question, hostname }) => {
    ...
    const effectiveHostname =
      readHostnameFromUrl(assistView.webContents.getURL()) ?? hostname ?? null;
    ...
    if (kind === 'typeahead') {
      ...
      if (enter.ok) recordRuleIfTeaching();
    ...
    if (result.ok) recordRuleIfTeaching();
    return { error: result.error?.message, ok: result.ok };
  },
  ...
  setActiveSection: section => {
    ...
    if (!desktopWindow || !appBrowserView || !assistView) return;
    layoutViews(desktopWindow, appBrowserView, assistView);
  },
...
void submitClient
  ...
  .then(rules => {
    if (rules.length > 0) {
      ...
      console.log(
        `[field-rules] hydrated ${rules.length} rules from server`,
      );
    }
  })
  .catch(error => {
...
const desktopSubmitRunner: DesktopSubmitRunner = {
  cancelRun: async () => {
  ...
  runLead: async request => {
    ...
    const writeRunJsonlSummary = async (
      result: DesktopSubmitLeadResult,
    ): Promise<void> => {
      if (!runJsonlLog || runJsonlSummaryWritten) return;
    ...
    const submitRunKey =
      request.mode === 'submit' ? buildSubmitRunKey(request) : null;
    if (submitRunKey && activeSubmitRunKeys.has(submitRunKey)) {
      const result = createDuplicateSubmitResult({
        applicationUrl: request.applicationUrl,
        jobLeadId: request.jobLeadId,
        message:
          'Skipped submit: another desktop submit run is already active for this application.',
        mode: request.mode,
      });
    ...
    try {
      if (request.mode === 'submit') {
        ...
        if (submittedCheck.alreadySubmitted) {
          const result = createDuplicateSubmitResult({
            ...
            message:
              submittedCheck.reason === 'existing_submission'
      ...
      console.log(
        `[desktop-submit] provider=${provider.id} readiness=${provider.readiness} runner=${provider.runner ?? 'none'}`,
      );
      ...
      if (preflightVerdict.closed) {
        ...
        const result: DesktopSubmitLeadResult = {
          applicationUrl: request.applicationUrl,
          executionEnvironment: 'DESKTOP_CDP',
          jobLeadId: request.jobLeadId,
          message: `Greenhouse posting is unavailable (${
            preflightVerdict.detectedPhrase ??
      ...
      const shouldTailor =
        Boolean(request.jobLeadId) &&
        profile.useOptimizedResumeOnSubmit === true;
      const tailorPromise = shouldTailor
        ? submitClient
            .tailorResumeForLead({ leadId: request.jobLeadId as string })
            .catch(error => {
              console.warn(
                'Parallel resume tailor failed; falling back to stored resume:',
                error,
              );
              return null;
            })
      ...
      const prepareTailoredResume = tailorPromise
        ? async () => {
            ...
            if (!tailored?.formats?.pdf) return null;
            try {
              ...
              console.warn(
                'Failed to materialize tailored resume PDF; falling back:',
                error,
              );
              return null;
            }
      ...
      const result = await runner(
        runToolRegistry,
        {
          ...
          lookupRecentVerificationCode: digits =>
            lookupVerificationCodeCached(digits, controller.signal),
        },
        { signal: controller.signal },
      );
      ...
      if (
        ...
        if (postflightVerdict.closed) {
          ...
          const overridden = {
            ...result,
            message: `Greenhouse posting is unavailable (${
              postflightVerdict.detectedPhrase ??
      ...
      if (
        ...
        if (successVerdict.confirmed) {
          const overridden: DesktopSubmitLeadResult = {
            ...result,
      ...
      if (runJsonlLog && !runJsonlSummaryWritten) {
        ...
        try {
          ...
          console.warn(
            '[run-jsonl-log] error summary write failed:',
            summaryError,
          );
        } finally {
          await runJsonlLog.close().catch(closeError => {
  ...
  swapAssistResumeFile: async request => {
    ...
    try {
      ...
      const injected = (await assistView.webContents.executeJavaScript(
        buildResumeFileInjectionScript({
          base64: bytes.base64,
      ...
      return injected
        ...
        : {
            injected: false,
            reason:
              'No matching <input type="file"> was found on the assist page.',
          };
    } catch (error) {
  ...
  tailorResumeForLead: request =>
    submitClient.tailorResumeForLead({ leadId: request.leadId }),
  runSmokeTest: (request: DesktopSmokeTestRequest) =>
    runProviderSmokeTest(request),
...
function emitSmokeProgress(event: DesktopSmokeTestProgressEvent) {
  if (!desktopWindow || desktopWindow.isDestroyed()) return;
  desktopWindow.webContents.send(
    DESKTOP_SUBMIT_IPC_EVENTS.smokeProgress,
    event,
  );
}
...
async function runProviderSmokeTest(
  ...
  const providerInfo = DESKTOP_RUNTIME_PROVIDERS.find(
    p => p.id === request.runtimeProviderId,
  );
  if (!providerInfo) {
    throw new Error(
      `Unknown runtime provider: ${request.runtimeProviderId}`,
    );
  }
  ...
  const runs: DesktopSmokeTestRun[] = [];
  ...
  try {
    for (let index = 0; index < request.count; index += 1) {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Random pick failed.';
        runs.push({
          applicationUrl: '',
          company: null,
          durationMs: 0,
      ...
      if (lead.jobListingId) excludeListingIds.push(lead.jobListingId);
      if (lead.company) excludeCompanies.push(lead.company);
...
async function runLeadForSmoke(
...
async function writeSmokeTestReport(input: {
  providerInfo: DesktopRuntimeProvider;
  request: DesktopSmokeTestRequest;
  runs: readonly DesktopSmokeTestRun[];
  startedAt: Date;
  endedAt: Date;
}): Promise<string> {
...
async function pickAvailableRandomLead(
  ...
  let lastUnavailable: DesktopRandomGreenhouseLead | null = null;

  for (
    let attempt = 0;
    attempt < RANDOM_PICK_AVAILABILITY_ATTEMPTS;
    attempt += 1
  ) {
...
async function persistUserActionFeedback(
  report: ReturnType<typeof parseUserActionReport>,
): Promise<void> {
  ...
  if (corrected.length === 0 && filled.length === 0) return;
  ...
  for (const field of [...corrected, ...filled]) {
    ...
    if (question.length < 6 || answer.length === 0) continue;
    try {
      addFieldRule({
        question,
        answer,
...
function inferDeterministicAnswer(
  query: {
    readonly question: string;
    readonly fieldType?: string;
    readonly options?: readonly string[];
  },
  profile: Awaited<ReturnType<typeof submitClient.syncProfileToIdentity>>,
): {
  ...
  if (
    ...
    if (
      ...
      return {
        ...
        confidence: 'high',
        reasoning:
          'US-authorized profile, sponsorship question targeting US — answering No.',
      };
    }
  }
  ...
  // a saved rule.
  if (
    /\bnon[-\s]?compete\b/.test(question) ||
    /\bnon[-\s]?solicitation\b/.test(question) ||
    /\bpost[-\s]?employment\s+(?:restriction|agreement|covenant|obligation)\b/.test(
      question,
    ) ||
    /\brestrictive\s+covenant\b/.test(question) ||
    /\bbound\s+by.*\b(?:agreement|contract|covenant|nda|restriction)\b/.test(
      question,
    ) ||
    (/\bagreed\s+to\b/.test(question) &&
      /\b(?:restriction|non[-\s]?compete|non[-\s]?solicitation)\b/.test(
        question,
      ))
  ) {
    return {
      ...
      confidence: 'high',
      reasoning:
        'Post-employment restriction question — defaulting to No (job-seeker assumption).',
    };
  }
  ...
  if (
    /\bauthor(?:ized|ization)\b.*\bwork\b/.test(question) ||
    ...
    if (isUsBasedAuthorizedProfile(profile) && questionTargetsUs(question)) {
      return {
        ...
        confidence: 'high',
        reasoning:
          'US-authorized profile, work-auth question targeting US — answering Yes.',
      };
    }
  }
...
function isUsBasedAuthorizedProfile(
  profile: Awaited<ReturnType<typeof submitClient.syncProfileToIdentity>>,
): boolean {
  ...
  return (
    authorization.includes('citizen') ||
...
function pickRunnerForProvider(
  provider: DesktopRuntimeProvider,
): typeof runGreenhouseSubmitLead | typeof runGenericSubmitLead {
...
async function readAssistSelectOptions(
  registry: ReturnType<typeof createDesktopToolRegistry>,
  selector: string,
  options?: { readonly query?: string },
): Promise<ReadonlyArray<{ readonly label: string; readonly value: string }>> {
  ...
  // Scope the option search to the listbox actually associated with the
  // clicked element. Without scoping we pick up [role="option"] elements
  // anywhere on the page — Greenhouse's phone-number country picker
  // renders ~250 country options at all times, and they bleed into every
  // unrelated select dropdown's option list.
  ...
  const collect = (
    scope: HtmlNode,
    options: { readonly filterBaseline: boolean },
  ): void => {
    if (!scope) return;
    ...
    for (const candidate of candidates) {
      ...
      if (options.filterBaseline && baselineKeys.has(key)) continue;
  ...
  if (seen.size === 0) {
    const portalRoots = root.querySelectorAll(
      '[role="listbox"], [class*="select__menu"], [class*="-menu-"], [class*="MenuList"]',
    );
...
const SUCCESS_PHRASE_PATTERNS: ReadonlyArray<RegExp> = [
  /thank(?:s|\s*you|-you)/i,
...
interface SubmissionSuccessVerdict {
  readonly confirmed: boolean;
  readonly detectedPhrase: string | null;
}

async function checkSubmissionSuccessFromAssistView(): Promise<SubmissionSuccessVerdict> {
  if (!assistView) {
  ...
  try {
    ...
    const liveBody = await assistView.webContents
      .executeJavaScript(
        '(function () { try { return document.body && document.body.innerText ? document.body.innerText.slice(0, 4000) : ""; } catch (_) { return ""; } })();',
        true,
      )
      .catch(() => '');
...
async function checkClosedPostingFromAssistView(): Promise<ClosedPostingVerdict> {
  if (!assistView) {
    return { closed: false, detectedPhrase: null, reason: null };
  }
  try {
    const html = (await assistView.webContents.executeJavaScript(
      'document.documentElement.outerHTML',
      true,
    )) as string;
    const title = (await assistView.webContents.executeJavaScript(
      'document.title',
      true,
    )) as string;
...
async function reportLeadUnavailable(input: {
  ...
  readonly verdict: ClosedPostingVerdict;
}): Promise<void> {
  if (!input.jobLeadId && !input.jobListingId) return;
  try {
...
async function detectAndRecordConfirmation(input: {
  ...
  let pageHtml: string;
  try {
    pageHtml = (await webContents.executeJavaScript(
      'document.documentElement.outerHTML',
      true,
    )) as string;
  } catch (error) {
  ...
  const providerConfirmation = detectProviderConfirmation(
...
async function recordDesktopRunResult(
  request: { readonly applicationUrl: string; readonly jobLeadId?: string },
  result: DesktopSubmitLeadResult,
): Promise<void> {
...
async function captureFailureSnapshot(
  status: DesktopSubmitLeadResult['status'],
): Promise<
  | {
...
> {
  ...
  try {
    const page = (await webContents.executeJavaScript(
      ...
      true,
    )) as { html?: unknown; height?: unknown; width?: unknown };
...
function clampSnapshotDimension(value: unknown, fallback: number): number {
...
function readHostnameFromUrl(url: string): string | null {
  try {
...
function readPersistedAssistUrl(): string | undefined {
...
function readPanelSize(value: unknown): number | undefined {
...
function readApplicationHostname(url: string): string | null {
  try {
...
// Ring buffer of recent page console error/warning events from the assist
// webContents. Drained into the run log when writeRunLog fires so admins
// can correlate a failed submit with whatever the page itself was logging
// at the time (form-validation errors, missing-resource warnings, etc).
interface PageConsoleEvent {
...
const pageConsoleBuffer: PageConsoleEvent[] = [];

function capturePageConsoleEvent(
  event: Omit<PageConsoleEvent, 'capturedAt'>,
): void {
  pageConsoleBuffer.push({ ...event, capturedAt: new Date().toISOString() });
  if (pageConsoleBuffer.length > PAGE_CONSOLE_BUFFER_LIMIT) {
    pageConsoleBuffer.splice(
      0,
      pageConsoleBuffer.length - PAGE_CONSOLE_BUFFER_LIMIT,
    );
  }
}

function drainPageConsoleBuffer(): readonly PageConsoleEvent[] {
  const snapshot = pageConsoleBuffer.slice();
  pageConsoleBuffer.length = 0;
  return snapshot;
}

function getFirstFailedToolName(
  result: DesktopSubmitLeadResult,
): string | undefined {
  return result.toolCalls.find(call => !call.ok)?.tool;
}

function getFirstFailedToolMessage(
  result: DesktopSubmitLeadResult,
): string | undefined {
...
async function writeRunLog(
  request: { readonly applicationUrl: string; readonly jobLeadId?: string; readonly mode: string },
  result: DesktopSubmitLeadResult,
): Promise<void> {
  try {
...
async function createWindow() {
  ...
  const ensureEyeSaver = () => {
    if (!shellState.isEyeSaverMode) return;
  ...
  let pageChangedTimer: NodeJS.Timeout | null = null;
  const emitPageChanged = () => {
    if (pageChangedTimer) clearTimeout(pageChangedTimer);
  ...
  const applyWindowTitle = (title: string) => {
    if (mainWindow.isDestroyed()) return;
    const trimmed = title.trim();
    mainWindow.setTitle(
      trimmed ? `${trimmed} — Gimme Job` : 'Gimme Job Desktop',
    );
  };
...
async function setAssistEyeSaverMode(
...
async function loadOptionalAssistUrl(
  webContents: WebContents,
  url: string,
): Promise<void> {
  try {
    ...
    console.warn(
      `Assist view initial navigation failed for ${url}: ${readNavigationError(error)}`,
    );
  }
}
...
function layoutViews(
  ...
  if (
    (activeSection !== 'training' && activeSection !== 'admin') ||
  ...
  if (activeSection === 'admin') {
    const appTopChrome =
      DESKTOP_APP_HEADER_HEIGHT + DESKTOP_ADMIN_BROWSER_CHROME_HEIGHT;
    appView.setBounds({
...
function readNavigationError(error: unknown): string {
