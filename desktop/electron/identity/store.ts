...
import type { IdentityKey, IdentityStore } from './types.js';
 ...
 */
export function createMacOSIdentityStore(
  opts: { readonly service?: string; readonly accountPrefix?: string } = {},
): IdentityStore {
  const service = opts.service ?? IDENTITY_KEYCHAIN_SERVICE;
  const accountPrefix =
    opts.accountPrefix ?? IDENTITY_KEYCHAIN_ACCOUNT_PREFIX;

  const accountFor = (key: IdentityKey) => `${accountPrefix}${key}`;
  ...
  return {
    ...
    async list() {
      const entries: { key: IdentityKey; hasValue: boolean }[] = [];
    ...
    async write(rawKey, value) {
      ...
      if (!ok) {
        throw new Error(
          `identity: value for "${key}" failed validation: ${reason}`,
        );
      }
      await execFileAsync('security', [
...
export function createMemoryIdentityStore(
  initial: Partial<Record<IdentityKey, string>> = {},
): IdentityStore {
  const values = new Map<IdentityKey, string>();
  for (const [key, value] of Object.entries(initial)) {
    if (value !== undefined) {
      ...
      if (!result.ok) {
        throw new Error(
          `identity: initial value for "${validated}" failed validation: ${result.reason}`,
        );
      }
  ...
  return {
    ...
    async write(rawKey, value) {
      ...
      if (!result.ok) {
        throw new Error(
          `identity: value for "${key}" failed validation: ${result.reason}`,
        );
      }
...
export function createIdentityStore(
  opts: {
    ...
    readonly initial?: Partial<Record<IdentityKey, string>>;
  } = {},
): IdentityStore {
 ...
 */
export async function loadIdentityValue(
  store: IdentityStore,
  rawKey: string,
): Promise<string> {
  const key = assertIdentityKey(rawKey);
  ...
  if (value === null) {
    throw new Error(
      `identity: key "${key}" is known but not populated. Open the Identity admin UI to seed it.`,
    );
  }
  return value;
}
