...
import type {
  ...
  DesktopAssistPageFieldAutofill,
  DesktopAssistPageIssue,
} from './types.js';

interface CollectAssistPageContextInput {
  readonly lastSubmitResult: DesktopSubmitLeadResult | null;
  readonly webContents: WebContents;
}

interface RawAssistPageContext {
  readonly fields?: readonly DesktopAssistPageField[];
  readonly title?: string;
  readonly url?: string;
}

export async function collectAssistPageContext({
  lastSubmitResult,
  webContents,
}: CollectAssistPageContextInput): Promise<DesktopAssistPageContext> {
  const rawContext = await readPageContext(webContents);
  const fields = annotateAutofilledFields(
    sanitizeFields(rawContext.fields),
    lastSubmitResult,
  );
  ...
  } satisfies DesktopAssistPageContext;
...
export async function highlightAssistPageField(
  ...
  try {
    return Boolean(
      await webContents.executeJavaScript(
        createFieldHighlightScript(selector),
        true,
      ),
    );
  } catch {
...
async function readPageContext(
  ...
  try {
    ...
    return value as RawAssistPageContext;
  } catch {
    return {
...
function sanitizeFields(
  fields: readonly DesktopAssistPageField[] | undefined,
): readonly DesktopAssistPageField[] {
  ...
  // get dropped.
  const RECOGNIZED_UPLOAD_LABEL =
    /\b(resume|cv|curriculum\s*vitae|cover\s*letter|portfolio|transcript|writing\s*sample|work\s*sample)\b/i;
  const labelsByNonFile = new Set<string>();
  for (const field of fields) {
    ...
    if (label) labelsByNonFile.add(label);
  }
  const deduped = fields.filter(field => {
    ...
    if (RECOGNIZED_UPLOAD_LABEL.test(label)) return true;
  ...
  return deduped.slice(0, 120).map(field => ({
    ...
    candidateSelectors: Array.isArray(field.candidateSelectors)
      ...
      : [],
    checked:
      typeof field.checked === 'boolean' || field.checked === null
        ? field.checked
        : null,
    ...
    shouldAvoid:
      typeof field.shouldAvoid === 'boolean' ? field.shouldAvoid : undefined,
...
function annotateAutofilledFields(
  fields: readonly DesktopAssistPageField[],
  lastSubmitResult: DesktopSubmitLeadResult | null,
): readonly DesktopAssistPageField[] {
  if (!lastSubmitResult) {
    return fields;
  }
  ...
  return fields.map(field => {
    ...
    return {
      ...
      } satisfies DesktopAssistPageFieldAutofill,
    };
  });
}

function derivePageIssues(
  context: Omit<DesktopAssistPageContext, 'issues'>,
): readonly DesktopAssistPageIssue[] {
  const issues: DesktopAssistPageIssue[] = [];
  ...
  const failedCalls =
    context.lastSubmitResult?.toolCalls.filter(call => !call.ok) ?? [];
  for (const call of failedCalls.slice(-5)) {
    issues.push({
      fieldSelector: call.selector,
      kind: 'tool-error',
      message: `${call.tool} failed${call.reason ? ` while trying to ${call.reason}` : ''}: ${
...
function createFieldHighlightScript(selector: string): string {
  return `
(() => {
