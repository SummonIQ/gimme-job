...
async function killExistingDesktopProcesses() {
    ...
    const portResult = spawnSync('lsof', ['-tiTCP:' + RENDERER_PORT, '-sTCP:LISTEN'], { encoding: 'utf8' });
    ...
    // this very script.
    const psResult = spawnSync('ps', ['-ax', '-o', 'pid=,command='], { encoding: 'utf8' });
    if (psResult.status === 0) {
        for (const line of psResult.stdout.split('\n')) {
            ...
            if (!Number.isFinite(pid) || pid === process.pid)
                continue;
            if (command.includes(projectMarker))
                victims.add(pid);
        }
    }
    if (victims.size === 0)
        return;
    console.log(`[dev] killing ${victims.size} stale desktop process(es): ${[...victims].join(', ')}`);
    ...
    const stillBound = spawnSync('lsof', ['-tiTCP:' + RENDERER_PORT, '-sTCP:LISTEN'], { encoding: 'utf8' });
