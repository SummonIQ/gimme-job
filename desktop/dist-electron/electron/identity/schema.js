 ...
 */
const nonEmpty = (value) => value.trim().length === 0 ? 'value is empty' : null;
...
const matchesPhone = (value) => {
    ...
    if (digits.length < 7)
        return 'must contain at least 7 digits';
    return null;
};
const matchesFilePath = (value) => {
    if (value.trim().length === 0)
        return 'value is empty';
...
export const IDENTITY_SCHEMA = Object.freeze({
    address_line_1: {
...
export const IDENTITY_KEYS = Object.freeze(Object.keys(IDENTITY_SCHEMA));
...
export function assertIdentityKey(key) {
    if (!isIdentityKey(key)) {
        throw new Error(`identity: unknown key "${String(key)}". Known keys: ${IDENTITY_KEYS.join(', ')}`);
    }
    return key;
}
export function validateIdentityValue(key, value) {
