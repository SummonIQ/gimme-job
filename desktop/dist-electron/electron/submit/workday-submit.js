...
import { call, failed, loadRequiredIdentity, readDomSnapshotHtml, readDomSnapshotTitle, readDomSnapshotUrl, resolveRemainingFieldsWithLlm, setCurrentRunSignal, sleep, waitForSubmissionConfirmation, } from './greenhouse-submit.js';
const STEP_PATTERNS = [
    {
        step: 'submitted',
        patterns: [
...
const APPLY_MANUALLY_SELECTORS = [
    'button[data-automation-id="applyManually"]',
    'a[data-automation-id="applyManually"]',
];
const MAX_STEP_ITERATIONS = 14;
const STEP_SETTLE_MS = 1500;
export async function runWorkdaySubmitLead(registry, request, options = {}) {
...
export function isWorkdayApplicationUrl(url) {
    return /\.myworkdayjobs\.com/i.test(url);
}
function createWorkdayRuntime(request) {
    return {
        async run(input) {
...
async function runWorkdayRuntime(request, input) {
    if (!request.continueFromCurrentPage) {
        const navigateResult = await call(input, 'navigate', { url: request.applicationUrl }, 'open Workday application page');
    ...
    if (overlaysDismissed > 0) {
        console.log(`[workday-submit] dismissed ${overlaysDismissed} consent overlay(s)`);
    }
    ...
    for (let iteration = 0; iteration < MAX_STEP_ITERATIONS; iteration += 1) {
        ...
        const snapshot = await call(input, 'dom_snapshot', {}, `Workday step detection (iter ${iteration + 1})`);
        if (!snapshot.ok)
        ...
        console.log(`[workday-submit] iteration ${iteration + 1}: detected step ${step}`);
        if (step === 'submitted') {
            return {
                message: 'Workday submission confirmed (post-submit summary page detected).',
                status: 'completed',
            };
        }
        ...
        if (step === 'sign_in_choice') {
            ...
            if (!clicked) {
                return {
                    message: 'Workday landed on the apply-method choice page but no "Apply Manually" control was found.',
        ...
        if (step === 'review') {
            if (request.mode !== 'submit') {
                return {
                    message: 'Training run reached Workday review page (submit not clicked).',
                    status: 'completed',
                };
            }
            const submitClicked = await clickFirstMatching(input, SUBMIT_BUTTON_SELECTORS);
            if (!submitClicked) {
                return {
                    message: 'Workday review page reached but the Submit button was not found.',
            ...
            if (!confirmation.confirmed) {
                return {
                    message: `Workday Submit fired but no confirmation page within ${confirmation.timeoutMs}ms.${confirmation.diagnostic ? ` ${confirmation.diagnostic}` : ''}`,
...
async function tryClickApply(input) {
    ...
    const applyButton = root.querySelector('a[data-automation-id="adventureButton"]') ??
        root.querySelector('button[data-automation-id="adventureButton"]') ??
        null;
    if (!applyButton)
        return;
    await call(input, 'click', { selector: 'a[data-automation-id="adventureButton"], button[data-automation-id="adventureButton"]' }, 'open Workday apply wizard');
}
async function tryResumeUpload(input, filePath, attemptedSelectors) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'Workday resume input lookup');
    if (!snapshot.ok)
        return;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
        return;
    const root = parse(html);
    const resumeInput = root.querySelector('input[data-automation-id="file-upload-input-ref"]') ??
    ...
    const selector = automationId
        ...
        : 'input[type="file"]';
    await call(input, 'upload', { filePath, selector }, 'attach Workday resume');
    attemptedSelectors.add(selector);
}
async function clickFirstMatching(input, selectors) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'Workday CTA discovery');
    if (!snapshot.ok)
        return false;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
    ...
    for (const selector of selectors) {
        ...
        if (!exists)
            continue;
        const result = await call(input, 'click', { selector }, `Workday click ${selector}`);
        if (result.ok)
            return true;
    }
    return false;
}
function detectWorkdayStep(html, title, url) {
    const root = parse(html);
    const headings = root
        .querySelectorAll('h1, h2, h3, [data-automation-id*="pageHeader" i], [data-automation-id*="title" i]')
