...
const GREENHOUSE_FIRST_NAME_SELECTOR = 'input#first_name,input[name="first_name"]';
...
const GREENHOUSE_CITY_SELECTOR = 'input#city,input[name="city"],textarea#city,textarea[name="city"]';
const GREENHOUSE_COUNTRY_SELECTOR = 'select#country,select[name="country"],select[name*="country" i],input#country,input[name="country"],input[name*="country" i],textarea#country,textarea[name="country"],textarea[name*="country" i]';
const GREENHOUSE_LOCATION_SELECTOR = 'input#location,input[name="location"],textarea#location,textarea[name="location"]';
...
const GREENHOUSE_RESUME_SELECTOR = 'input[type="file"][name="resume"],input[type="file"]#resume,input[type="file"][name*="resume" i]:not([name*="cover" i]),input[type="file"][id*="resume" i]:not([id*="cover" i]),input[type="file"][aria-label*="resume" i]:not([aria-label*="cover" i])';
const GREENHOUSE_COVER_LETTER_SELECTOR = 'input[type="file"][name*="cover" i],input[type="file"][id*="cover" i],input[type="file"][aria-label*="cover" i]';
const GREENHOUSE_STATE_SELECTOR = 'input#state,input[name="state"],textarea#state,textarea[name="state"],select#state,select[name="state"]';
const GREENHOUSE_SUBMIT_SELECTOR = 'button#submit_app,button#submit-application,button[type="submit"][data-qa="submit-application"],button.template-btn-submit,form#application-form button[type="submit"],form#application_form button[type="submit"],form[id*="application" i] button[type="submit"],input[type="submit"]';
const GREENHOUSE_QUESTION_AUTHORIZED_SELECTOR = 'select#question_authorized,select[name="question_authorized"]';
const GREENHOUSE_QUESTION_SPONSORSHIP_SELECTOR = 'select#question_sponsorship,select[name="question_sponsorship"]';
const PREFERRED_NAME_QUESTION_PATTERN = /\bpreferred[\s_-]+(?:first[\s_-]+)?name\b|\bnickname\b|\bchosen[\s_-]+name\b|what\s+should\s+we\s+call\s+you|\bgo\s+by\b/i;
...
export async function runGreenhouseSubmitLead(registry, request, options = {}) {
...
function readToolSelector(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input)) {
        return undefined;
    }
    return readString(input.selector) ?? undefined;
}
function readString(value) {
    return typeof value === 'string' && value.trim() ? value.trim() : null;
}
function createGreenhouseRuntime(request) {
    return {
        async run(input) {
...
async function runGreenhouseRuntime(request, input) {
    if (!request.continueFromCurrentPage) {
        const navigateResult = await call(input, 'navigate', { url: request.applicationUrl }, 'open Greenhouse application');
        if (!navigateResult.ok)
            return failed(navigateResult);
    }
    const readyResult = await call(input, 'wait_for', { selector: GREENHOUSE_FIRST_NAME_SELECTOR, timeoutMs: 15_000 }, 'wait for Greenhouse form');
    if (!readyResult.ok)
    ...
    if (overlaysDismissed > 0) {
        console.log(`[greenhouse-submit] dismissed ${overlaysDismissed} consent overlay(s)`);
    }
    ...
    try {
        ...
        if (request.prepareTailoredResume) {
            try {
                ...
                if (tailoredPath)
                    earlyResumeFilePath = tailoredPath;
            }
            catch (error) {
                console.warn('[greenhouse-submit] tailored resume prep failed (early), falling back', error);
            }
        }
        const earlyUpload = await attemptResumeUpload(input, earlyResumeFilePath, resolvedSelectors.resume ?? GREENHOUSE_RESUME_SELECTOR, 'attach resume (early)');
        if (earlyUpload.ok) {
    ...
    if (uploadFailureReason) {
        console.warn('[greenhouse-submit] early resume upload skipped:', uploadFailureReason);
    }
    const fillResults = [
        await call(input, 'fill', {
            selector: GREENHOUSE_FIRST_NAME_SELECTOR,
            value: identity.firstName,
        }, 'fill first name'),
        await call(input, 'fill', { selector: GREENHOUSE_LAST_NAME_SELECTOR, value: identity.lastName }, 'fill last name'),
        await call(input, 'fill', { selector: GREENHOUSE_EMAIL_SELECTOR, value: identity.email }, 'fill email'),
        await call(input, 'fill', { selector: GREENHOUSE_PHONE_SELECTOR, value: identity.phone }, 'fill phone'),
    ];
    const failedFill = fillResults.find(result => !result.ok);
    if (failedFill)
        return failed(failedFill);
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
        ...
        const phoneValueResult = await call(input, 'read_element', { selector: GREENHOUSE_PHONE_SELECTOR }, 'verify phone value');
        if (!phoneValueResult.ok)
            break;
        const data = phoneValueResult.data;
        const actualPhone = data && typeof data.value === 'string' ? data.value : null;
        if (!actualPhone ||
        ...
        await call(input, 'fill', { selector: GREENHOUSE_PHONE_SELECTOR, value: identity.phone }, 'refill phone after mismatch');
    }
    if (!phoneStable) {
        const typed = await typePhoneDigitsByKeystroke(input, GREENHOUSE_PHONE_SELECTOR, identity.phone);
        console.warn(`[greenhouse-submit] phone keystroke fallback ${typed ? 'succeeded' : 'still mismatched'}`);
    }
    ...
    if (!resumeUploadDone) {
        ...
        if (request.prepareTailoredResume) {
            try {
                const tailoredPath = await request.prepareTailoredResume();
                if (tailoredPath)
                    resumeFilePath = tailoredPath;
            }
            catch (error) {
                console.warn('[greenhouse-submit] tailored resume prep failed, falling back', error);
            }
        }
        const uploadResult = await attemptResumeUpload(input, resumeFilePath, resolvedSelectors.resume ?? GREENHOUSE_RESUME_SELECTOR, 'attach resume');
        if (uploadResult.ok) {
            resumeUploadDone = true;
        }
        else {
            ...
            console.warn('[greenhouse-submit] resume upload skipped — continuing without it:', uploadResult.error?.message ?? 'unknown');
        }
    }
    ...
    if (request.mode === 'submit') {
        ...
        if (remainingBeforeOptional.length === 0) {
            const guardResult = await call(input, 'submit_guard', { enabled: false }, 'owner-approved submit mode (early)');
            if (guardResult.ok) {
                const earlySubmit = await call(input, 'click', { selector: GREENHOUSE_SUBMIT_SELECTOR }, 'submit application (early — required fields filled)');
                if (earlySubmit.ok) {
                    const confirmation = await waitForSubmissionConfirmation(input);
                    if (confirmation.confirmed) {
                        return {
                            message: 'Submit confirmed on first pass — required fields were sufficient.',
                            status: 'completed',
                        };
                    }
    ...
    const optionalFieldResult = await fillOptionalGreenhouseFields(input, request.aiProvider, profileWithInferredGender, resolvedSelectors, request.resolveUnknownFieldAnswer, request.lookupRecentVerificationCode);
    ...
    if (request.mode === 'submit') {
        const guardResult = await call(input, 'submit_guard', { enabled: false }, 'owner-approved submit mode');
        if (!guardResult.ok)
            return failed(guardResult);
    }
    const submitResult = await call(input, 'click', { selector: GREENHOUSE_SUBMIT_SELECTOR }, request.mode === 'submit' ? 'submit application' : 'verify submit guard');
    if (!submitResult.ok) {
        ...
        if (request.mode === 'training' &&
            ...
            const message = remaining.length === 0
                ...
                : `Training run stopped at submit guard with ${remaining.length} required field${remaining.length === 1 ? '' : 's'} still empty: ${remaining
                    .slice(0, 3)
    ...
    if (request.mode === 'submit') {
        ...
        if (!confirmation.confirmed && request.lookupRecentVerificationCode) {
            ...
            if (challenge) {
                const handled = await resolveVerificationCodeChallenge(input, challenge, request.lookupRecentVerificationCode);
                if (handled.ok) {
                    const resubmit = await call(input, 'click', { selector: GREENHOUSE_SUBMIT_SELECTOR }, 'submit application (after verification code)');
                    if (resubmit.ok) {
        ...
        if (!confirmation.confirmed) {
            ...
            const validationDetail = validationErrors.length > 0
                ? ` Validation error${validationErrors.length === 1 ? '' : 's'}: ${validationErrors
                    ...
                    .join('; ')}${validationErrors.length > 3 ? ` (+${validationErrors.length - 3} more)` : ''}.`
                : '';
            return {
                message: `Submit click fired but Greenhouse did not show a confirmation page within ${confirmation.timeoutMs}ms.${detail}${validationDetail}${confirmation.diagnostic ? ` ${confirmation.diagnostic}` : ''}`,
                status: 'failed',
            };
        }
        return {
            message: 'Submit confirmed — Greenhouse showed an application-received page.',
            status: 'completed',
        };
    }
    return {
...
const SUBMISSION_CONFIRMATION_URL_PATTERNS = [
...
const CONFIRMATION_CONTAINER_PATTERNS = [
    /thank|received|submitted|success|confirm|appli/i,
];
export async function waitForSubmissionConfirmation(input) {
    ...
    const initialSnapshot = await call(input, 'dom_snapshot', {}, 'capture pre-confirmation URL');
    const initialUrl = readDomSnapshotUrl(initialSnapshot.ok ? initialSnapshot.data : null);
    ...
    while (Date.now() < deadline) {
        const snapshot = await call(input, 'dom_snapshot', {}, 'verify post-submit confirmation');
        if (snapshot.ok) {
            ...
            if (html) {
                ...
                const submitStillVisible = root.querySelector(GREENHOUSE_SUBMIT_SELECTOR);
                const formStillVisible = root.querySelector('form#application_form, form#application-form, form[id*="application" i]');
                ...
                const matchedConfirmationText = SUBMISSION_CONFIRMATION_TEXT_PATTERNS.some(pattern => pattern.test(haystack));
                const matchedConfirmationUrl = url
                    ? SUBMISSION_CONFIRMATION_URL_PATTERNS.some(pattern => pattern.test(url))
                    : false;
                ...
                const matchedConfirmationContainer = root
                    .querySelectorAll('[class*="confirm" i], [class*="thank" i], [class*="submitted" i], [class*="success" i], [id*="confirm" i], [id*="thank" i]')
                    .some(node => {
                    const text = (node.text ?? '').trim();
                    return (text.length > 0 &&
                        CONFIRMATION_CONTAINER_PATTERNS.some(pattern => pattern.test(text)));
                });
                const navigatedAway = !!initialUrl && !!url && stripQuery(url) !== stripQuery(initialUrl);
                if (matchedConfirmationText ||
                ...
                if (lastFormVisible && !navigatedAway && lastBodyHead === prevBodyHead) {
                    ...
                    if (stableNegativeTicks >= STABLE_NEGATIVE_TICKS_TO_EXIT)
                        break;
                }
                else {
    ...
    const diagnostic = `[at timeout] url=${lastUrl ?? 'unknown'}` +
        ` | title=${lastTitle ?? 'unknown'}` +
        ` | formVisible=${lastFormVisible}` +
        ` | body="${lastBodyHead}"`;
    return { confirmed: false, timeoutMs, diagnostic };
}
async function detectUnansweredRequiredFields(input) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'final required-empty check');
    if (!snapshot.ok)
        return [];
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
        return [];
    const root = parse(html);
    const candidates = root.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="image"]):not([type="reset"]):not([type="search"]):not([type="file"]), textarea, select, [role="combobox"], [aria-haspopup="listbox"], [aria-haspopup="menu"]');
    const seen = new Set();
    const unanswered = [];
    for (const candidate of candidates) {
        ...
        const isCustomCombobox = role === 'combobox' || isInsideCustomSelectWidget(candidate);
        const label = readGreenhouseLabelText(root, candidate);
        if (!label)
            continue;
        if (!detectFieldRequired(root, candidate, label))
            continue;
        const value = candidate.getAttribute('value') ?? '';
        const innerText = candidate.text?.trim() ?? '';
        if (isCustomCombobox && readCustomSelectSelectedText(candidate))
            continue;
        if (tagName === 'select' &&
            value.trim() &&
        ...
        if (tagName === 'select' &&
            ...
            const selected = candidate
                ...
                ?.getAttribute('value');
            if (selected && !/^select/i.test(selected))
                continue;
        }
        if (!isCustomCombobox &&
            tagName !== 'select' &&
...
export async function detectValidationErrors(input) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'detect validation errors');
    if (!snapshot.ok)
        return [];
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
    ...
    for (const selector of errorSelectors) {
        ...
        for (const node of matches) {
            ...
            if (/©|copyright|terms|privacy|cookie/i.test(text))
                continue;
...
async function fillPreferredNameFields(input, preferredName) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'scan preferred name fields');
    if (!snapshot.ok)
        return;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
        return;
    const root = parse(html);
    const candidates = root.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="image"]):not([type="reset"]):not([type="search"]):not([type="file"]), textarea');
    const seen = new Set();
    for (const candidate of candidates) {
        const selector = selectorForNode(candidate);
        if (!selector || seen.has(selector))
        ...
        seen.add(selector);
        const label = readGreenhouseLabelText(root, candidate);
        if (!PREFERRED_NAME_QUESTION_PATTERN.test(label))
            continue;
        const value = candidate.getAttribute('value') ?? '';
        const innerText = candidate.text?.trim() ?? '';
        if ((value && value.trim()) || innerText)
            continue;
        const result = await call(input, 'fill', { selector, value: preferredName }, 'fill preferred name');
        if (result.ok)
            await sleep(60);
    }
}
...
export async function loadOptionalContactUrls(input) {
    const linkedin = await loadIdentityValue(input, 'linkedin_url').catch(() => ({ ok: false }));
    const github = await loadIdentityValue(input, 'github_url').catch(() => ({
        ok: false,
    }));
    const website = await loadIdentityValue(input, 'website_url').catch(() => ({ ok: false }));
    return {
...
async function loadGreenhouseFieldSelectors(input) {
    const snapshotResult = await call(input, 'dom_snapshot', {}, 'inspect Greenhouse form fields');
...
async function fillOptionalGreenhouseFields(input, aiProvider, identity, selectors, resolveUnknownFieldAnswer, lookupRecentVerificationCode) {
    ...
    const locationRequirementValue = inferLocationRequirementValue(identity.country);
    const canadaLocationPreferenceValue = inferCanadaLocationPreferenceValue(identity.canadaWorkPreference ?? null, identity.country);
    const authorizationValue = inferAuthorizationValue(identity.workAuthorization, identity.country);
    const sponsorshipValue = inferSponsorshipValue(identity.sponsorshipRequired, selectors.questionSponsorshipLabel, identity.country);
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        ...
        const shouldDeferHispanic = wantsRace && !raceVisible && iteration < MAX_RACE_WAIT_ITERATIONS;
        const plan = [
            fillOptionalFieldBySelector(input, currentSelectors.country ?? GREENHOUSE_COUNTRY_SELECTOR, {
            ...
            fillLocationTypeahead(input, currentSelectors.locationCity ?? GREENHOUSE_LOCATION_SELECTOR, locationValue ?? identity.city, 'fill location city', 'locationCity'),
            fillOptionalFieldBySelector(input, currentSelectors.state ?? GREENHOUSE_STATE_SELECTOR, { selectValue: normalizedStateCode, textValue: normalizedStateText }, 'fill state', 'state'),
            fillOptional(input, currentSelectors.cityAndStateQuestion, locationValue, 'fill city and state response', 'cityAndStateQuestion'),
            selectOptional(input, currentSelectors.canadaLocationPreference, canadaLocationPreferenceValue, 'select Canada location preference', 'canadaLocationPreference'),
            fillOptional(input, currentSelectors.linkedin, identity.linkedinUrl, 'fill linkedin profile', 'linkedin'),
            fillOptional(input, currentSelectors.github, identity.githubUrl, 'fill github profile', 'github'),
            fillOptional(input, currentSelectors.website, identity.websiteUrl, 'fill website', 'website'),
            selectOptionalWithFallback(input, currentSelectors.referralSource, [
                ...
                'Web search',
                'Job board',
            ], 'select referral source', 'referralSource'),
            selectOptional(input, currentSelectors.questionAuthorized ??
                GREENHOUSE_QUESTION_AUTHORIZED_SELECTOR, authorizationValue, 'select work authorization', 'questionAuthorized'),
            selectOptional(input, currentSelectors.questionSponsorship ??
                GREENHOUSE_QUESTION_SPONSORSHIP_SELECTOR, sponsorshipValue, 'select sponsorship requirement', 'questionSponsorship'),
            selectOptional(input, currentSelectors.locationRequirement, locationRequirementValue, 'select location requirement', 'locationRequirement'),
            selectOptional(input, currentSelectors.exportComplianceCitizenship, identity.citizenshipStatus ?? null, 'select export compliance citizenship status', 'exportComplianceCitizenship'),
            ...selectOptionalAll(input, currentSelectors.gender, identity.gender, 'select gender', 'gender'),
            // Race must come before hispanic — choosing "No" on hispanic hides
            // the race field on some Greenhouse layouts (Zeta).
            ...selectOptionalAll(input, currentSelectors.race, identity.race, 'select race', 'race'),
            ...(shouldDeferHispanic
                ? []
                : selectOptionalAll(input, currentSelectors.hispanicLatino, identity.hispanicLatino, 'select hispanic latino', 'hispanicLatino')),
            ...selectOptionalAll(input, currentSelectors.veteranStatus, identity.veteranStatus, 'select veteran status', 'veteranStatus'),
            ...selectOptionalAll(input, currentSelectors.disabilityStatus, identity.disabilityStatus, 'select disability status', 'disabilityStatus'),
            fillOptional(input, currentSelectors.desiredSalary, identity.salaryExpectation, 'fill desired salary', 'desiredSalary'),
            fillOptional(input, currentSelectors.residence, locationValue, 'fill current residence', 'residence'),
        ];
        ...
        const seenPlanSelectors = new Set();
        ...
        const remaining = plan.filter((item) => {
            if (item === null)
                return false;
            ...
            if (attempted.has(key))
                return false;
            if (attemptedSelectors.has(item.selector))
                return false;
            if (seenPlanKeys.has(key))
                return false;
            if (seenPlanSelectors.has(item.selector))
                return false;
        ...
        if (remaining.length === 0) {
            ...
            if (!shouldDeferHispanic) {
                stableTicks++;
                if (stableTicks >= STABLE_TICKS_TO_EXIT)
                    break;
            }
            await sleep(250);
            continue;
    ...
    const automationDisclosureChallenge = await findAutomationDisclosureChallenge(input, attemptedSelectors);
    ...
    if (resolveUnknownFieldAnswer) {
        await resolveRemainingFieldsWithLlm(input, resolveUnknownFieldAnswer, attemptedSelectors, aiProvider, lookupRecentVerificationCode);
    }
    return null;
}
async function findAutomationDisclosureChallenge(input, attemptedByPlanner) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'scan anti-bot disclosure fields');
    if (!snapshot.ok)
        return null;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
...
const AUTOMATION_DISCLOSURE_SELECTOR_PATTERNS = [
    /\bbot[_-]?check\b/i,
    /\banti[_-]?bot\b/i,
    /\bcaptcha\b/i,
    /\bhuman[_-]?check\b/i,
...
const AUTOMATION_DISCLOSURE_TEXT_PATTERNS = [
...
function isAutomationDisclosureChallenge(field) {
    if (AUTOMATION_DISCLOSURE_SELECTOR_PATTERNS.some(pattern => pattern.test(field.selector))) {
        return true;
    }
    ...
    const asksIdentity = /best\s+describes\s+you|are\s+you|identify\s+as/.test(text);
    const namesAutomation = /\b(ai|bot|automated|automation|program)\b/.test(text);
    const namesHuman = /\bhuman\b/.test(text);
    return asksIdentity && namesAutomation && namesHuman;
}
const VERIFICATION_CODE_TEXT_PATTERNS = [
...
function isUrlListField(field) {
    if (field.fieldType !== 'textarea' && field.fieldType !== 'text')
        return false;
    const text = field.question.toLowerCase();
    if (!text)
        return false;
    return /(other\s+(?:links|urls|websites)|additional\s+(?:links|urls|websites)|where\s+else|personal\s+websites?\s+\(?other|other\s+social\s+media|portfolio\s+links?|other\s+profiles)/i.test(text);
}
function isVerificationCodeField(field) {
    if (field.fieldType !== 'text' && field.fieldType !== 'unknown')
        return false;
    const text = field.question.toLowerCase();
    if (!text)
        return false;
...
function inferVerificationCodeDigits(field) {
...
async function detectVerificationCodeChallenge(input) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'detect verification-code challenge');
    if (!snapshot.ok)
        return null;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
        return null;
    const fields = collectUnansweredQuestions(html, new Set());
    for (const field of fields) {
        if (!isVerificationCodeField(field))
            continue;
        return {
    ...
    const looksLikeChallenge = VERIFICATION_CODE_TEXT_PATTERNS.some(p => p.test(bodyText));
    if (!looksLikeChallenge)
        return null;
    const inputs = root.querySelectorAll('input[type="text"]:not([disabled]), input[type="tel"]:not([disabled]), input:not([type]):not([disabled])');
    for (const candidate of inputs) {
        const maxLen = Number.parseInt(candidate.getAttribute('maxlength') ?? '', 10);
...
// session.
async function resolveVerificationCodeChallenge(input, challenge, lookupRecentVerificationCode) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
        const lookup = await lookupRecentVerificationCode(challenge.expectedDigits ?? undefined);
        if (lookup && lookup.code) {
            const fillResult = await call(input, 'fill', { selector: challenge.selector, value: lookup.code }, `post-submit verification-code fill (${challenge.question.slice(0, 60)})`);
            console.log(`[greenhouse-submit] post-submit verification-code → ${fillResult.ok
        ...
        if (attempt === 0) {
            console.log(`[greenhouse-submit] verification-code challenge detected; waiting for inbox email (poll every ${RETRY_MS}ms, up to ${(MAX_ATTEMPTS * RETRY_MS) / 60000}min)…`);
        }
        else if (attempt % 15 === 0) {
            console.log(`[greenhouse-submit] still waiting for verification-code email (${attempt * RETRY_MS / 1000}s elapsed)…`);
        }
...
export async function resolveRemainingFieldsWithLlm(input, resolveUnknownFieldAnswer, attemptedByPlanner, aiProvider, lookupRecentVerificationCode) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'scan unanswered fields');
    if (!snapshot.ok)
        return;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
    ...
    const fields = sortFieldsByVisualPosition(collectUnansweredQuestions(html, attemptedByPlanner), positions);
    console.log(`[greenhouse-submit] LLM fallback: ${fields.length} unanswered required field(s) detected`, fields.map(f => ({
    ...
    const withTimeout = (promise, label) => new Promise(resolve => {
        ...
        const timer = setTimeout(() => {
            if (settled)
                return;
            settled = true;
            console.warn(`[greenhouse-submit] LLM timeout after ${LLM_TIMEOUT_MS}ms on "${label.slice(0, 80)}" — skipping`);
            resolve(null);
        }, LLM_TIMEOUT_MS);
        promise
            .then(value => {
    ...
    for (const field of fields) {
        ...
        if (prefetched.has(key))
            continue;
        prefetched.set(key, withTimeout(resolveUnknownFieldAnswer({
            aiProvider,
    ...
    for (const field of fields) {
        if (lookupRecentVerificationCode && isVerificationCodeField(field)) {
            ...
            for (let attempt = 0; attempt < MAX_LOOKUP_ATTEMPTS; attempt += 1) {
                lookup = await lookupRecentVerificationCode(expectedDigits ?? undefined);
                if (lookup && lookup.code)
                    break;
                if (attempt < MAX_LOOKUP_ATTEMPTS - 1) {
                    console.log(`[greenhouse-submit] verification-code attempt ${attempt + 1} empty, polling inbox in ${LOOKUP_RETRY_MS}ms…`);
                    await sleep(LOOKUP_RETRY_MS);
                }
            }
            if (lookup && lookup.code) {
                const fillResult = await call(input, 'fill', { selector: field.selector, value: lookup.code }, `verification-code fill (${field.question.slice(0, 80)})`);
                console.log(`[greenhouse-submit] verification-code "${field.question.slice(0, 60)}" → ${fillResult.ok
                    ? `filled (${lookup.digits} digits from ${lookup.fromEmail || 'inbox'})`
                    : 'FAILED: ' + (fillResult.error?.message ?? 'unknown')}`);
                if (fillResult.ok) {
                    await sleep(150);
                ...
                console.log(`[greenhouse-submit] verification-code "${field.question.slice(0, 60)}": no recent code in inbox after ${MAX_LOOKUP_ATTEMPTS} attempts${expectedDigits ? ` (expected ${expectedDigits} digits)` : ''}`);
            }
        }
        ...
        const answer = prefetchedAnswer
            ...
            : await withTimeout(resolveUnknownFieldAnswer({
                aiProvider,
                fieldType: field.fieldType,
                options: liveOptions,
                question: field.question,
        ...
        console.log(`[greenhouse-submit] LLM answer for "${field.question.slice(0, 60)}"`, {
        ...
        if (!answer || !answer.answer.trim()) {
            console.log(`[greenhouse-submit] skip "${field.question.slice(0, 60)}": empty answer`);
            continue;
        }
        ...
        const isStrictOption = field.fieldType === 'select' ||
            field.fieldType === 'radio' ||
            field.fieldType === 'checkbox';
        if (isStrictOption && answer.confidence === 'low') {
            console.log(`[greenhouse-submit] skip "${field.question.slice(0, 60)}": low confidence on ${field.fieldType}`);
            continue;
        }
        ...
        if (field.fieldType === 'select' && liveOptions.length > 0) {
            ...
            if (!matched) {
                console.log(`[greenhouse-submit] skip select "${field.question.slice(0, 60)}": answer "${answer.answer.slice(0, 60)}" doesn't match any of ${liveOptions.length} option(s): ${liveOptions.slice(0, 6).join(' | ')}`);
                continue;
            }
            valueToFill = matched;
        }
        ...
        const tool = isCheckbox
            ...
            : field.fieldType === 'select'
                ...
                : 'fill';
        if (isCheckbox) {
            const declined = /\b(no|decline|skip|do not|don'?t|never)\b/i.test(answer.answer);
            if (declined) {
                console.log(`[greenhouse-submit] skip checkbox "${field.question.slice(0, 60)}": LLM answered "${answer.answer.slice(0, 40)}"`);
                continue;
            }
        }
        ...
        const fillResult = tool === 'click'
            ? await call(input, 'click', { selector: field.selector }, fillReason)
            : tool === 'fill'
                ? await fillWithVerification(input, field.selector, valueToFill, fillReason)
                : await call(input, tool, { selector: field.selector, value: valueToFill }, fillReason);
        console.log(`[greenhouse-submit] LLM ${tool} "${field.question.slice(0, 60)}" → ${fillResult.ok ? 'ok' : 'FAILED: ' + (fillResult.error?.message ?? 'unknown')}`);
        await sleep(70);
    }
}
async function emitFormSnapshot(input, request) {
    if (!request.onFormSnapshot)
        return;
    const snapshot = await call(input, 'dom_snapshot', {}, 'capture form snapshot');
    if (!snapshot.ok)
        return;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
    ...
    const root = parse(html);
    const detected = [];
    const candidates = root.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="image"]):not([type="reset"]):not([type="search"]), textarea, select, [role="combobox"], [aria-haspopup="listbox"], [aria-haspopup="menu"]');
    const seen = new Set();
    for (const candidate of candidates) {
        const selector = selectorForNode(candidate);
        if (!selector || seen.has(selector))
        ...
        const role = candidate.getAttribute('role') ?? '';
        const isCombobox = role === 'combobox' || isInsideCustomSelectWidget(candidate);
        const fieldType = tagName === 'select' || isCombobox
            ? 'select'
            : tagName === 'textarea'
                ? 'textarea'
                : type === 'radio'
    ...
    for (const field of detected) {
        let options = field.options;
        if (field.fieldType === 'select' && options.length === 0) {
            options = (await readLiveSelectOptions(input, field.selector)).map(option => ({ label: option, value: option }));
        }
...
function detectFieldRequired(root, candidate, label) {
    ...
    if (id) {
        ...
        if (labelEl) {
            const indicator = labelEl.querySelector('[class*="required" i], [aria-label*="required" i]');
            if (indicator)
                return true;
            if (/\*|\(required\)/i.test(labelEl.text ?? ''))
                return true;
        }
    }
    const parentLabel = candidate.closest('label');
    if (parentLabel) {
        const indicator = parentLabel.querySelector('[class*="required" i], [aria-label*="required" i]');
        if (indicator)
            return true;
        if (/\*|\(required\)/i.test(parentLabel.text ?? ''))
            return true;
    }
    return false;
}
async function readFieldValue(input, field) {
    try {
        const result = await call(input, 'read_element', { selector: field.selector }, `read filled value for ${field.label}`);
        if (!result.ok)
            return '';
        ...
        if (field.fieldType === 'checkbox' || field.fieldType === 'radio') {
            const checked = data.attributes?.['checked'] ?? data.attributes?.['aria-checked'] ?? '';
...
export async function readLiveSelectOptionsShared(input, selector) {
    return readLiveSelectOptions(input, selector);
}
async function readLiveSelectOptions(input, selector) {
    ...
    const baseline = await call(input, 'dom_snapshot', {}, 'snapshot baseline options before open');
    const baselineTexts = new Set();
    if (baseline.ok) {
        const baselineHtml = readDomSnapshotHtml(baseline.data);
        if (baselineHtml) {
            const baselineRoot = parse(baselineHtml);
            const baselineCandidates = baselineRoot.querySelectorAll('[role="option"], [class*="select__option"], [class*="-option-"], li[id*="option"], [data-option-value]');
    ...
    const open = await call(input, 'click', { selector }, 'open dropdown to read options');
    if (!open.ok)
        return [];
    await sleep(220);
    const live = await call(input, 'dom_snapshot', {}, 'snapshot live dropdown options');
    if (!live.ok)
        return [];
    ...
    await call(input, 'press_key', { key: 'Escape' }, 'close opened dropdown after option read');
    await sleep(80);
    return Array.from(optionTexts);
}
function answerMatchesAnyOption(answer, options) {
...
function findMatchingOption(answer, options) {
    ...
    for (const option of options) {
        ...
        if (!optionNormalized)
            continue;
        if (optionNormalized.includes(normalized) ||
    ...
    const answerTokens = new Set(normalized
        .split(/[^a-z0-9+#-]+/)
...
export function inferGenderFromFirstName(firstName) {
    const normalized = firstName.trim().toLowerCase().split(/[\s-]/)[0];
    if (!normalized)
        return null;
    if (STRONG_MALE_NAMES.has(normalized))
        return 'Male';
    if (STRONG_FEMALE_NAMES.has(normalized))
        return 'Female';
    return null;
}
...
function augmentFollowUpQuestion(root, candidate, question) {
    if (!FOLLOW_UP_PREFIX_PATTERN.test(question))
        return question;
...
function findPrecedingAnsweredField(root, candidate) {
    const targetSelector = selectorForNode(candidate);
    if (!targetSelector)
        return null;
    const candidates = root.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="image"]):not([type="reset"]), textarea, select, [role="combobox"]');
    ...
    for (const node of candidates) {
        ...
        if (sel === targetSelector)
            break; // Reached the candidate; stop.
        const tagName = node.tagName?.toLowerCase() ?? '';
        const role = node.getAttribute('role') ?? '';
        const isCustomCombobox = role === 'combobox' || isInsideCustomSelectWidget(node);
        let answer = '';
        if (tagName === 'select') {
...
function collectUnansweredQuestions(html, attemptedByPlanner) {
    const root = parse(html);
    const candidates = root.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="image"]):not([type="reset"]), textarea, select, [role="combobox"], [aria-haspopup="listbox"], [aria-haspopup="menu"]');
    const seen = new Set();
    const unanswered = [];
    for (const candidate of candidates) {
        const tagName = candidate.tagName.toLowerCase();
        ...
        if (attemptedByPlanner.has(selector))
            continue;
        const value = candidate.getAttribute('value');
        const innerText = candidate.text?.trim() ?? '';
        const isCustomCombobox = role === 'combobox' || isInsideCustomSelectWidget(candidate);
        ...
        if (isToggleInput) {
            ...
            if (hasValue && tagName !== 'select' && !isCustomCombobox)
                continue;
        }
        if (tagName === 'select') {
        ...
        if (!rawQuestion || rawQuestion.length < 4)
            continue;
        ...
        // didn't want.
        if (!detectFieldRequired(root, candidate, rawQuestion))
            continue;
        ...
        const selectorIsStandardBasic = /input#(?:first_name|last_name|email|phone)\b/i.test(selector) ||
            /\binput\[name=("|')(?:first_name|last_name|email|phone)\1\]/i.test(selector);
        const questionIsExactBasic = /^(?:email(?:\s+address)?|phone(?:\s+number)?|first\s+name|last\s+name|resume|linkedin(?:\s+url)?|linkedin\s+profile)$/i.test(question);
        if (selectorIsStandardBasic || questionIsExactBasic) {
            continue;
        }
        const fieldType = tagName === 'select' || isCustomCombobox
            ? 'select'
            : tagName === 'textarea'
                ? 'textarea'
                : type === 'radio'
                    ...
                    : type === 'checkbox'
                        ? 'checkbox'
                        : 'text';
        const options = fieldType === 'select' ? readSelectOptionTexts(root, candidate) : [];
        unanswered.push({
            fieldType,
            options,
            question,
...
function isInsideCustomSelectWidget(candidate) {
    let node = candidate.parentNode;
    for (let depth = 0; depth < 5 && node; depth++) {
        const hasAttr = typeof node.getAttribute === 'function';
        if (hasAttr) {
...
function readCustomSelectSelectedText(candidate) {
    let node = candidate.parentNode;
    for (let depth = 0; depth < 5 && node; depth++) {
        const selectedNodes = node.querySelectorAll('[class], [aria-selected="true"], [data-selected], [data-value]');
        for (const selectedNode of selectedNodes) {
            const cls = selectedNode.getAttribute('class') ?? '';
            const isSelectedValue = /single.?value|multi.?value|selected.?value|select__value|selected/i.test(cls) || selectedNode.getAttribute('aria-selected') === 'true';
            if (!isSelectedValue)
                continue;
            const text = normalizeFieldLabelText(selectedNode.text ?? '');
            if (text && !isSelectPlaceholderText(text))
                return text;
        }
        node = node.parentNode;
    }
...
function isSelectPlaceholderText(value) {
    return /^(select|choose|please select|search|type to search|start typing|\.\.\.)/i.test(value.trim());
}
function readSelectOptionTexts(root, candidate) {
    return readSelectOptionDetails(root, candidate).map(option => option.label);
}
function readSelectOptionDetails(root, candidate) {
    // Native <select>
...
function readFieldPlaceholder(candidate) {
    const placeholder = normalizeFieldLabelText(candidate.getAttribute('placeholder') ?? '');
...
async function readAlreadyFilledSelectorSet(input) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'snapshot pre-fill state');
    if (!snapshot.ok)
        return null;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
    ...
    const candidates = root.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="image"]):not([type="reset"]):not([type="file"]), textarea, select, [role="combobox"]');
    for (const candidate of candidates) {
        const selector = selectorForNode(candidate);
        if (!selector)
            continue;
        ...
        if (type === 'checkbox' || type === 'radio') {
            const isChecked = candidate.hasAttribute('checked') ||
                candidate.getAttribute('aria-checked') === 'true';
            if (isChecked)
                filled.add(selector);
            continue;
...
function fillOptional(input, selector, value, reason, dedupeKey) {
...
function fillLocationTypeahead(input, selector, value, reason, dedupeKey) {
    if (!selector || !value?.trim())
        return null;
    ...
    const OPTION_SELECTOR = '[role="option"], .pac-item, .select__option, [class*="suggestion"]';
    return {
        dedupeKey,
        selector,
        run: async () => {
            ...
            const fillResult = await call(input, 'fill', { selector, value: trimmed }, reason);
            if (!fillResult.ok)
                return fillResult;
            // Give the suggestion list a chance to populate from the API.
            const waitResult = await call(input, 'wait_for', { selector: OPTION_SELECTOR, timeoutMs: 3500 }, `${reason}: wait for typeahead options`);
            if (!waitResult.ok) {
                // No options surfaced — fall back to the plain fill result and let
                // downstream validation handle it.
                return fillResult;
            }
            const committed = await commitTypeaheadOption(input, selector, trimmed, reason);
            if (!committed) {
                // Keyboard fallback for older Greenhouse vintages where the
                // listbox isn't directly click-targetable.
                await call(input, 'press_key', { key: 'ArrowDown' }, `${reason}: highlight (keyboard fallback)`);
                await call(input, 'press_key', { key: 'Enter' }, `${reason}: select (keyboard fallback)`);
            }
            return fillResult;
        },
    };
...
async function commitTypeaheadOption(input, inputSelector, expectedValue, reason) {
    const expectedLower = expectedValue.toLowerCase();
    const expectedFirstToken = expectedLower.split(/[,\s]+/).filter(Boolean)[0] ?? expectedLower;
    for (let attempt = 0; attempt < 8; attempt += 1) {
        if (attempt > 0)
            await sleep(250);
        const snapshot = await call(input, 'dom_snapshot', {}, `${reason}: locate typeahead option`);
        if (!snapshot.ok)
            continue;
        const html = readDomSnapshotHtml(snapshot.data);
        if (!html)
        ...
        const lookupById = (id) => root.querySelector(`[id="${cssEscapeAttr(id)}"]`);
        const pushIdsFrom = (attr) => {
            if (!attr)
                return;
            for (const id of attr.split(/\s+/).filter(Boolean)) {
        ...
        for (const scope of scopes) {
            ...
            const clickResult = await call(input, 'click', { selector: optSelector }, `${reason}: click typeahead option "${optText}"`);
            if (!clickResult.ok)
                continue;
            await sleep(150);
...
function selectorForOptionElement(element) {
...
function selectOptional(input, selector, value, reason, dedupeKey) {
    if (!selector || !value?.trim())
        return null;
    return {
        dedupeKey,
...
function selectOptionalWithFallback(input, selector, values, reason, dedupeKey) {
    if (!selector)
        return null;
    ...
    return {
        ...
        run: async () => {
            ...
            for (const value of candidates) {
                const result = await call(input, 'select', { selector, value }, `${reason} → "${value}"`);
                if (result.ok)
                    return result;
                lastResult = result;
            }
...
function selectOptionalAll(input, selectors, value, reason, dedupeKeyPrefix) {
    if (!value?.trim() || selectors.length === 0)
        return [];
    return selectors.map((selector, index) => {
        const reasonForSelector = selectors.length > 1 ? `${reason} (#${index + 1})` : reason;
        return {
            dedupeKey: dedupeKeyPrefix ? `${dedupeKeyPrefix}[${index}]` : undefined,
            selector,
            run: () => call(input, 'select', { selector, value: value.trim() }, reasonForSelector),
        };
    });
}
function emptyGreenhouseFieldSelectors() {
    return {
...
function normalizeCountrySelectValue(country) {
    if (!country)
        return null;
...
function normalizeCountryTextValue(country) {
...
function normalizeStateCode(state) {
    ...
    const matchingEntry = Object.entries(US_STATE_LABELS).find(([, label]) => label.toLowerCase() === state.trim().toLowerCase());
    return matchingEntry?.[0] ?? state.trim();
}
function normalizeStateTextValue(state) {
    const normalizedCode = normalizeStateCode(state);
    if (!normalizedCode)
        return null;
    return US_STATE_LABELS[normalizedCode] ?? state?.trim() ?? null;
}
function fillOptionalFieldBySelector(input, selector, value, reason, dedupeKey) {
    if (!selector)
        return null;
    if (selector.trim().toLowerCase().startsWith('select')) {
        return selectOptional(input, selector, value.selectValue, reason, dedupeKey);
    }
    ...
    return {
        ...
        selector,
        run: async () => {
            const selectResult = await call(input, 'select', { selector, value: preferredValue.trim() }, `${reason} (try select)`);
            if (selectResult.ok)
                return selectResult;
            ...
            if (/not found|did not match|select option not found|select target not found/i.test(message) &&
                fallbackValue?.trim()) {
                return call(input, 'fill', { selector, value: fallbackValue.trim() }, `${reason} (fill fallback)`);
            }
            return selectResult;
        },
    };
}
function inferAuthorizationValue(workAuthorization, country) {
...
function inferLocationRequirementValue(country) {
    if (!country)
        return null;
    return /^(us|usa|united states)$/i.test(country.trim()) ? 'yes' : 'no';
}
function inferCanadaLocationPreferenceValue(canadaWorkPreference, country) {
    const normalizedPreference = canadaWorkPreference?.trim().toLowerCase();
    if (normalizedPreference) {
        if (['yes', 'y', 'true', '1'].includes(normalizedPreference))
            return 'yes';
        if (['no', 'n', 'false', '0'].includes(normalizedPreference))
            return 'no';
...
function inferSponsorshipValue(sponsorshipRequired, questionLabel, userCountry) {
    ...
    if (targetCountry && userCountry) {
        if (!countriesMatch(targetCountry, userCountry))
            return 'yes';
    }
...
function extractCountryFromSponsorshipLabel(label) {
    if (!label)
        return null;
    // Match "...sponsorship to work for X in Canada?" / "...visa to work in the US?"
    const match = label.match(/\bin\s+(?:the\s+)?(united states|usa|us|america|canada|united kingdom|uk|england|britain|ireland|germany|france|spain|italy|netherlands|australia|new zealand|japan|china|india|mexico|brazil|singapore|hong kong|south korea|argentina|chile|colombia)\b/i);
    return match?.[1]?.trim() ?? null;
}
function countriesMatch(a, b) {
    const normalize = (value) => {
        const lower = value.trim().toLowerCase();
        if (/^(us|usa|united states|america)$/.test(lower))
            return 'us';
        if (/^(uk|united kingdom|england|britain)$/.test(lower))
            return 'uk';
        if (/^(ca|can|canada)$/.test(lower))
            return 'ca';
        return lower;
    };
    return normalize(a) === normalize(b);
}
function buildLocationValue(city, state) {
    if (city && state)
        return `${city}, ${state}`;
    return city ?? state ?? null;
}
export function readDomSnapshotHtml(data) {
    if (!data || typeof data !== 'object' || Array.isArray(data))
        return null;
    const html = data.html;
    return typeof html === 'string' && html.length > 0 ? html : null;
}
export function readDomSnapshotTitle(data) {
    if (!data || typeof data !== 'object' || Array.isArray(data))
        return null;
    const title = data.title;
    return typeof title === 'string' && title.length > 0 ? title : null;
}
export function readDomSnapshotPositions(data) {
    if (!data || typeof data !== 'object' || Array.isArray(data))
        return {};
...
function sortFieldsByVisualPosition(fields, positions) {
...
function positionKeyForSelector(selector) {
...
export function readDomSnapshotUrl(data) {
    if (!data || typeof data !== 'object' || Array.isArray(data))
        return null;
    const url = data.url;
...
function findGreenhouseSelectorByLabel(root, input) {
    const matches = findGreenhouseSelectorsByLabel(root, input);
    return matches[0] ?? null;
}
function findGreenhouseSelectorsByLabel(root, input) {
    ...
    for (const candidate of candidates) {
        ...
        if (input.negativeLabelPatterns?.some(pattern => pattern.test(labelText))) {
            continue;
        }
        const selector = selectorForNode(candidate);
        if (selector && !seen.has(selector)) {
...
async function readCurrentSiblingUrls(input) {
...
function collectSiblingFormUrls(html) {
    const root = parse(html);
    const urls = new Set();
...
function findGreenhouseLabelTextByPatterns(root, input) {
    const candidates = root.querySelectorAll(input.tagNames.join(','));
    for (const candidate of candidates) {
        const labelText = readGreenhouseLabelText(root, candidate);
        if (labelText &&
...
function readGreenhouseLabelText(root, candidate) {
    const id = candidate.getAttribute('id');
    if (id) {
    ...
    const ariaLabel = normalizeFieldLabelText(candidate.getAttribute('aria-label') ?? '');
    if (ariaLabel)
        return ariaLabel;
    const placeholder = normalizeFieldLabelText(candidate.getAttribute('placeholder') ?? '');
    if (placeholder)
    ...
    return normalizeFieldLabelText(`${candidate.getAttribute('name') ?? ''} ${candidate.getAttribute('id') ?? ''}`);
}
function normalizeFieldLabelText(value) {
    return value.trim().replace(/\s+/g, ' ');
}
function selectorForNode(candidate) {
    const tagName = candidate.tagName.toLowerCase();
    const id = candidate.getAttribute('id');
    if (id) {
...
function isIgnorableMissingFieldError(message) {
    if (!message)
        return false;
    return /not found|did not match/i.test(message);
}
async function loadIdentityValue(input, key) {
...
async function attemptResumeUpload(input, filePath, selector, reason) {
    ...
    const snapshot = await call(input, 'dom_snapshot', {}, `${reason} (scan for attach trigger)`);
    ...
    if (!triggerSelector)
        return first;
    const triggerClick = await call(input, 'click', { selector: triggerSelector }, `${reason} (click attach-file trigger)`);
    if (!triggerClick.ok)
        return first;
    await sleep(300);
    return call(input, 'upload', { filePath, selector }, `${reason} (retry)`);
}
const RESUME_ATTACH_TRIGGER_PATTERN = /\b(attach\s+(?:a\s+)?file|attach\s+resume|attach\s+(?:cv|curriculum vitae)|upload\s+(?:a\s+)?(?:file|resume|cv)|enter\s+manually|choose\s+(?:a\s+)?file|browse\s+files?|select\s+file)\b/i;
function findResumeAttachTriggerSelector(html) {
    const root = parse(html);
    ...
    const candidates = root.querySelectorAll('button, a[role="button"], [role="button"], label, span, div');
    let resumeScoped = null;
    let anyScoped = null;
    for (const element of candidates) {
        const text = (element.text ?? '').trim();
        if (!text || text.length > 60)
            continue;
        if (!RESUME_ATTACH_TRIGGER_PATTERN.test(text))
            continue;
        const selector = selectorForAttachTrigger(element);
        if (!selector)
            continue;
        const ancestorText = collectAncestorText(element, 4).toLowerCase();
        const isResumeScoped = /\b(resume|cv|curriculum vitae)\b/.test(ancestorText) &&
            !/\bcover\s*letter\b/.test(ancestorText);
        if (isResumeScoped) {
            if (!resumeScoped)
                resumeScoped = selector;
        }
        else if (!anyScoped) {
            anyScoped = selector;
        }
...
function selectorForAttachTrigger(element) {
    const id = element.getAttribute('id');
    if (id)
        return `#${cssEscapeAttr(id)}`;
...
function collectAncestorText(element, depth) {
    let current = element;
    const parts = [];
...
export async function call(input, tool, toolInput, reason) {
    ...
    if (traceEnabled) {
        ...
        console.log(`[trace ${ms}ms] ${tool} ${summarizeTraceInput(toolInput)} (${reason}) → ${status}`);
    }
    return result;
}
...
export async function fillWithVerification(input, selector, value, reason) {
    ...
    console.warn(`[fill-verify] "${selector}" expected "${value.slice(0, 40)}" but read back "${(stuck ?? '').slice(0, 40)}" — retrying via keystrokes`);
    await call(input, 'click', { selector }, `${reason} (focus for retry)`);
    await sleep(60);
    await call(input, 'fill', { selector, value: '' }, `${reason} (clear for retry)`);
    await sleep(40);
    const retry = await call(input, 'fill', { selector, value }, `${reason} (retry)`);
    return retry;
}
async function readFieldValueBySelector(input, selector) {
    const snapshot = await call(input, 'dom_snapshot', {}, 'verify post-fill value');
    if (!snapshot.ok)
        return null;
    const html = readDomSnapshotHtml(snapshot.data);
    if (!html)
...
function valuesAreEquivalent(actual, expected) {
...
let autofillPausedResolvers = [];
let autofillPaused = false;
export function setAutofillPaused(paused) {
...
export function isAutofillPaused() {
    return autofillPaused;
}
async function waitIfAutofillPaused() {
    if (!autofillPaused)
        return;
    ...
    await new Promise((resolve, reject) => {
        ...
        const onAbort = () => {
            autofillPausedResolvers = autofillPausedResolvers.filter(r => r !== resolve);
            reject(new DesktopAgentCancelledError());
        };
...
async function typePhoneDigitsByKeystroke(input, selector, phone) {
    const digits = stripPhoneDigits(phone);
    if (!digits)
        return false;
    const focusResult = await call(input, 'click', { selector }, 'focus phone for keystroke fill');
    if (!focusResult.ok)
        return false;
    await call(input, 'fill', { selector, value: '' }, 'clear phone before keystroke fill');
    await sleep(150);
    ...
    await sleep(500);
    const verify = await call(input, 'read_element', { selector }, 'verify phone after keystroke fill');
    if (!verify.ok)
        return false;
...
function buildObjective(request) {
