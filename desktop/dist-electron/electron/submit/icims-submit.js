...
const ICIMS_FORM_INDICATOR_SELECTORS = [
    'input[name="firstname"]',
    'input[id*="firstname" i]',
    'input[id*="iCIMS_FirstName" i]',
    'form[id*="apply" i]',
];
const ICIMS_DISCLAIMER_AGREE_SELECTORS = [
    'button[name*="agree" i]',
    'input[name*="agree" i]',
...
export async function runIcimsSubmitLead(registry, request, options = {}) {
...
export function isIcimsApplicationUrl(url) {
    return /\.icims\.com/i.test(url);
}
function createIcimsRuntime(request) {
    return {
        async run(input) {
...
async function runIcimsRuntime(request, input) {
    if (!request.continueFromCurrentPage) {
        const navigateResult = await call(input, 'navigate', { url: request.applicationUrl }, 'open iCIMS application page');
    ...
    if (overlaysDismissed > 0) {
        console.log(`[icims-submit] dismissed ${overlaysDismissed} consent overlay(s)`);
    }
    ...
    for (let iteration = 0; iteration < MAX_STEP_ITERATIONS; iteration += 1) {
        await sleep(1000);
        const snapshot = await call(input, 'dom_snapshot', {}, `iCIMS step (${iteration + 1})`);
        if (!snapshot.ok)
        ...
        const advanced = await clickFirstMatching(input, ICIMS_DISCLAIMER_AGREE_SELECTORS, root);
        if (!advanced) {
            ...
            if (title && /already\s+applied|thank\s+you/i.test(title)) {
                return {
                    message: 'iCIMS reports candidate already applied (or post-confirmation page detected).',
                    status: 'completed',
                };
            }
            return {
                message: 'iCIMS landed on an unrecognized page (no form, no disclaimer). Open the page manually.',
    ...
    for (const selector of ICIMS_IDENTITY_SELECTORS.resume) {
        const result = await call(input, 'upload', { filePath: resumeFilePath, selector }, 'iCIMS resume upload');
        if (result.ok) {
    ...
    if (!submitSnapshot.ok)
        return failed(submitSnapshot);
    ...
    if (!confirmation.confirmed) {
        return {
            message: `iCIMS submit fired but no confirmation page within ${confirmation.timeoutMs}ms.${confirmation.diagnostic ? ` ${confirmation.diagnostic}` : ''}`,
...
async function fillBySelectors(input, selectors, value, reason, attemptedSelectors) {
...
async function clickFirstMatching(input, selectors, root) {
    for (const selector of selectors) {
        ...
        if (!exists)
            continue;
        const result = await call(input, 'click', { selector }, `iCIMS click ${selector}`);
        if (result.ok)
            return true;
    }
    return false;
