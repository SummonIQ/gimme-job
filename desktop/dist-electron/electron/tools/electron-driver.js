...
export function createElectronCdpToolDriver({ getWebContents, identityValues = {}, identityStore, }) {
    let isNetworkObservationEnabled = false;
    let isSubmitGuardEnabled = true;
    return {
        async click(input) {
            const result = await executeInPage(getWebContents(), `
        ...
        `);
            ...
            if (result.isSubmitIntent) {
                await executeInPage(getWebContents(), `
            const element = document.querySelector(${toJson(input.selector)});
        ...
        async cookiesGet(input) {
            const cookies = await getWebContents().session.cookies.get(input.url ? { url: input.url } : {});
            return { cookies };
        },
        ...
        async domSnapshot(_input) {
            ...
            return executeInPage(getWebContents(), `
          (function () {
            try {
        ...
        async networkObserve(input) {
            ...
            await webContents.debugger.sendCommand(input.enabled ? 'Network.enable' : 'Network.disable');
        ...
        async screenshotRegion(input) {
            ...
            const webContents = getWebContents();
            const image = await withAssistEyeSaverDisabled(webContents, () => webContents.capturePage(rectangle));
            return {
        ...
        async select(input) {
            const nativeSelectResult = await executeInPage(getWebContents(), `
        ...
        `);
            if (!nativeSelectResult.handled) {
                const trustedMouseHandled = await selectCustomOptionWithTrustedMouse(getWebContents(), input.selector, input.value);
                if (!trustedMouseHandled) {
                    await openCustomSelect(getWebContents(), input.selector);
                    await waitForCustomSelectOption(getWebContents(), input.selector, input.value, 2_000);
                    await clickCustomSelectOption(getWebContents(), input.selector, input.value);
                }
            }
            return { selector: input.selector, value: input.value };
        },
        ...
        async upload(input) {
            ...
            const isGreenhouseBoardsPage = currentUrl.includes('job-boards.greenhouse.io');
            ...
            if (isGreenhouseBoardsPage) {
                const handled = await uploadViaInterceptedFileChooser(webContents, input.selector, input.filePath);
                if (handled) {
            ...
            const result = await executeInPage(webContents, `
...
async function uploadViaInterceptedFileChooser(webContents, selector, filePath) {
    const debuggerApi = webContents.debugger;
    if (!debuggerApi)
        return false;
    const buttonRect = await executeInPage(webContents, `
    ...
    try {
        const documentResult = (await debuggerApi.sendCommand('DOM.getDocument', {}));
...
async function verifyGreenhouseResumeUpload(webContents, selector, filePath) {
    return executeInPage(webContents, `
...
async function setFileInputFiles(webContents, selector, filePath) {
    const debuggerApi = webContents.debugger;
    if (!debuggerApi) {
        return;
    }
    ...
    try {
        ...
        if (tagName !== 'input' || typeAttr !== 'file') {
            throw new Error(`Upload selector matched <${tagName || 'unknown'}${typeAttr ? ` type="${typeAttr}"` : ''}>, expected <input type="file">: ${selector}`);
        }
        await debuggerApi.sendCommand('DOM.setFileInputFiles', {
            files: [filePath],
            nodeId,
...
`;
async function selectCustomOptionWithTrustedMouse(webContents, selector, value) {
    const debuggerApi = webContents.debugger;
    if (!debuggerApi)
        return false;
    const controlPoint = await readCustomSelectControlClickPoint(webContents, selector);
    if (!controlPoint.found)
        return false;
    let didAttach = false;
    if (!debuggerApi.isAttached()) {
        debuggerApi.attach('1.3');
    ...
    try {
        await dispatchTrustedMouseClick(debuggerApi, controlPoint.x, controlPoint.y);
        ...
        if (optionPoint.found) {
            await dispatchTrustedMouseClick(debuggerApi, optionPoint.x, optionPoint.y);
            await sleep(160);
        ...
        const keyboardConfirmed = await selectCustomOptionWithKeyboard(webContents, debuggerApi, selector, value);
...
async function selectCustomOptionWithKeyboard(webContents, debuggerApi, selector, value) {
...
async function focusCustomSelectInput(webContents, selector) {
    await executeInPage(webContents, `
      ${RESOLVE_SELECT_CONTROL_SCRIPT}
...
async function dispatchKey(debuggerApi, key) {
...
async function isSelectValueApplied(webContents, selector, value) {
    return executeInPage(webContents, `
      ${RESOLVE_SELECT_CONTROL_SCRIPT}
...
async function readCustomSelectControlClickPoint(webContents, selector) {
    return executeInPage(webContents, `
    ${RESOLVE_SELECT_CONTROL_SCRIPT}
    const element = document.querySelector(${toJson(selector)});
    const control = resolveSelectControl(element);
...
async function readCustomSelectOptionClickPoint(webContents, selector, value) {
    return executeInPage(webContents, `
    ${SELECT_MATCHER_SCRIPT}
    const target = document.querySelector(${toJson(selector)});
...
async function dispatchTrustedMouseClick(debuggerApi, x, y) {
    await debuggerApi.sendCommand('Input.dispatchMouseEvent', {
        button: 'left',
        clickCount: 1,
        type: 'mousePressed',
...
async function openCustomSelect(webContents, selector) {
    await executeInPage(webContents, `
      ${RESOLVE_SELECT_CONTROL_SCRIPT}
      const element = document.querySelector(${toJson(selector)});
      if (!(element instanceof HTMLElement)) {
...
async function waitForCustomSelectOption(webContents, selector, value, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const ready = await executeInPage(webContents, `
        ${SELECT_MATCHER_SCRIPT}
        const target = document.querySelector(${toJson(selector)});
        const requestedValue = ${toJson(value)};
...
async function clickCustomSelectOption(webContents, selector, value) {
    await executeInPage(webContents, `
      ${SELECT_MATCHER_SCRIPT}
      const target = document.querySelector(${toJson(selector)});
      const requestedValue = ${toJson(value)};
...
async function isWaitMatch(webContents, input) {
    return executeInPage(webContents, `
...
function executeInPage(webContents, script) {
    const wrappedScript = `
    (() => {
      try {
  ...
  `;
    return webContents
        ...
        if (result &&
            ...
            if (!parsedResult.ok) {
                const message = parsedResult.error?.message ?? 'Desktop page evaluation failed.';
        ...
        throw new Error(`Script failed to execute in desktop webview at ${currentUrl}.`);
    });
}
